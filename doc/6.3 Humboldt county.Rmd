...
both trips originating in and ending in Humboldt County. We use the
new county wide weights of the Humboldt County to generate weighted
results for trips that originated in Humboldt County and statewide
...
Now, we first focus on trips ending in Humboldt County.  Here we use
different weights depending on the origin of the trip.  Specifically,
when the origin is Humboldt County, we use the county-specific
weights.  When the origin is outside Humboldt County, we use the
statewide weights.
...
names(place_23)
# Change the weight for non Humboldt County origin
# (Assign state wide weight for non Humboldt origin)
place_23$NTCFW <-
    ifelse((place_23$OCTFIP==23),
           place_23$NEXPTCFPERWGT,
           place_23$EXPTCFPERWGT)
head(place_23)
```

So `place_23$NTCFW` now contains the trips ending in Humboldt county
with appropriate expansion weights appended.  Now we can generate
observations for modal split for trips to Humboldt based on these
weights.
...
trips ending in Humboldt, by mode.
...
Now we perform a similar computation for trips *from* Humboldt County.
As before, we subset the place data, but this time for trips
originating in Humboldt County.
...
And lastly for the Humboldt origins, here is the drive alone modal
...
```{r hc-31,cache=TRUE}
#Import the activity level variables data frame and give a new name
library(chts2011)
act_data <- chts_act
names(act_data)
...
We start with trips from Humboldt County, subsetting based open the
origin county `OCTFIP=23`.
...
activities (and associated trips) originating in Humboldt County, and
...
Now we want to find trips ending in Humboldt county `(CTFIP=23)`.
Again, we subset the place table and adjust the weights to be Humboldt
weights if the origins are in Humboldt, and to use statewide weights
if the origins are outside Humboldt.
...
# (Assign state wide weight for non Humboldt origin)
act_23D$NTCFW <-
  ifelse((act_23D$OCTFIP==23),
         act_23D$NEXPTCFPERWGT,
         act_23D$EXPTCFPERWGT.y)
names(act_23D)
head(act_23D)
```
...
trips ending in Humboldt County.
