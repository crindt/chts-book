---
title: "Data Request for Humboldt County"
author: "Suman Mitra"
date: "September 3, 2015"
output: html_document
---

```{r hc-setup-0,echo=FALSE,cache=FALSE}
...
setwd(p("District 1/Humboldt County"))
...
Now, we first focus on trips ending in Humboldt County.  Here we use different 
weights depending on the origin of the trip.  Specifically, when the origin is Humboldt 
County, we use the county-specific weights.  When the origin is outside 
Humboldt County, we use the statewide weights.

```{r hc-3,cache=TRUE}
# Trips to Humboldt County
# Create a subset for trips to Humboldt county 
# We are only interested in trips to Humboldt County (CTFIP=23) 
# Create a subset for trips to Humboldt County 
place_23<-subset(nplacedata,CTFIP==23)
names(place_23)
# Change the weight for non Humboldt County origin (Assign state wide weight for non Humboldt origin)
place_23$NTCFW <- ifelse((place_23$OCTFIP==23), place_23$NEXPTCFPERWGT, place_23$EXPTCFPERWGT)
head(place_23)
```

So `place_23$NTCFW` now contains the trips ending in Humboldt county with appropriate 
expansion weights appended.  Now we can generate observations for modal split for trips 
to Humboldt based on these weights.

Notes: In the tables: Ob = Observations, Per = Percent, UW = Unweighted and W = Weighted.

```{r hc-4,cache=TRUE}
## Generate frequency for trip Mode
temp1<-as.data.frame(table(place_23$Mode))
...
temp2<-aggregate(NTCFW~Mode,FUN=sum,data=place_23)
...
temp3
## Save as csv file
write.csv(temp3,file="Mode_23d.csv",row.names=FALSE)
rm(temp1)
rm(temp2)
rm(temp3)
```
...
```{r hc-7,cache=TRUE}
## Generate frequency for trip Mode
temp1<-aggregate(TRIPDUR~Mode,FUN=sum,data=place_23)
## Create  percentage
...
```{r hc-8,cache=TRUE}
## Create weighted frequency
temp<-place_23$NTCFW*place_23$TRIPDUR
temp2<-aggregate(temp~Mode,FUN=sum,data=place_23)
## Create weighted percentage
...
temp3$avg_uw<-temp3$TDu_UW/Mode_23d$Ob_UW
temp3
temp3$avg_w<-temp3$TDu_W/Mode_23d$Ob_W
temp3
temp3[,1]<-as.character(temp3[,1])
temp3<- rbind(temp3, c("Total", colSums(temp3[,2:7],na.rm = TRUE)))
temp3
## Save as csv file
write.csv(temp3,file="TripDUrm_D23.csv",row.names=FALSE)
rm(temp1)
rm(temp2)
rm(temp3)
rm(temp)
...
Moving on, we determine Total Trip distance for all trips ending in Humboldt, by mode.

```{r hc-10,cache=TRUE}
## Generate frequency for trip Mode
temp1<-aggregate(TripDistance~Mode,FUN=sum,data=place_23)
## Create  percentage
...
```{r hc-11,cache=TRUE}
## Create weighted frequency
temp<-place_23$NTCFW*place_23$TripDistance
temp2<-aggregate(temp~Mode,FUN=sum,data=place_23)
## Create weighted percentage
temp2$Percent<-temp2$temp*100/sum(temp2$temp)
## Names
...
temp3$avg_uw<-temp3$TDis_UW/Mode_23d$Ob_UW
temp3
temp3$avg_w<-temp3$TDis_W/Mode_23d$Ob_W
temp3
temp3[,1]<-as.character(temp3[,1])
temp3<- rbind(temp3, c("Total", colSums(temp3[,2:7],na.rm = TRUE)))
temp3
## Save as csv file
write.csv(temp3,file="TripDistanceM_D23.csv",row.names=FALSE)
rm(temp1)
rm(temp2)
rm(temp3)
rm(temp)
...
```{r hc-13}
place_23d<-subset(place_23,(TOTTR==1 & (MODE==5|MODE==6|MODE==7|MODE==8|MODE==10)))
head(place_23d)
```
...
temp1<-as.data.frame(table(place_23d$Mode))
## Create percentage of frequency
temp1$Percent<-temp1$Freq*100/sum(temp1$Freq)
## Names 
names(temp1)
...
```{r hc-15}
## Create weighted frequency
temp2<-aggregate(NTCFW~Mode,FUN=sum,data=place_23d)
## Create weighted percentage
temp2$Percent<-temp2$NTCFW*100/sum(temp2$NTCFW)
## Names
names(temp2)
...
temp3
## Save as csv file
write.csv(temp3,file="Mode_23Ddrialn.csv",row.names=FALSE)
rm(temp1)
rm(temp2)
rm(temp3)
```
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Trips from Humboldt County')
```


Now we perform a similar computation for trips *from* Humboldt County.  As before, we
subset the place data, but this time for trips originating in Humboldt County.

```{r hc-17,cache=TRUE}

# Let us estimate trips from Humboldt county (OCTFIP=23) 
# Create a subset for trips from Humboldt county 
place_23O<-subset(nplacedata,OCTFIP==23)
names(place_23O)
head(place_23O)
```
...
Now we generate the mode frequency table for all trips from Humboldt County.

```{r hc-18,cache=TRUE}
# Generate observations for modal split for trips from Humboldt based on new weights
## Generate frequency for trip Mode
temp1<-as.data.frame(table(place_23O$Mode))
## Create percentage of frequency
temp1$Percent<-temp1$Freq*100/sum(temp1$Freq)
## Names 
names(temp1)
...
```{r hc-19,cache=TRUE}
## Create weighted frequency
temp2<-aggregate(NEXPTCFPERWGT~Mode,FUN=sum,data=place_23O)
## Create weighted percentage
...
temp3
## Save as csv file
write.csv(temp3,file="Mode_23O.csv",row.names=FALSE)
rm(temp1)
rm(temp2)
rm(temp3)
```
...
Now we repeat the trip duration calculation for Humboldt Origins.

```{r hc-21,cache=TRUE}
## Total Trip duration for all trips from Humboldt, by Mode
## Generate frequency for trip Mode
temp1<-aggregate(TRIPDUR~Mode,FUN=sum,data=place_23O)
## Create  percentage
temp1$Percent<-temp1$TRIPDUR*100/sum(temp1$TRIPDUR)
## Names
names(temp1)
...
```{r hc-22,cache=TRUE}
## Create weighted frequency
temp<-place_23O$NEXPTCFPERWGT*place_23O$TRIPDUR
temp2<-aggregate(temp~Mode,FUN=sum,data=place_23O)
## Create weighted percentage
temp2$Percent<-temp2$temp*100/sum(temp2$temp)
## Names
names(temp2)
...
temp3 <- temp3[,c(1,2,4,3,5)]
temp3
temp3$avg_uw<-temp3$TDu_UW/Mode_23O$Ob_UW
temp3
temp3$avg_w<-temp3$TDu_W/Mode_23O$Ob_W
temp3
temp3[,1]<-as.character(temp3[,1])
temp3<- rbind(temp3, c("Total", colSums(temp3[,2:7],na.rm = TRUE)))
temp3
...
Here is the trip distance calculation for all trips from Humboldt, by Mode.

```{r hc-24,cache=TRUE}
## Generate frequency for trip Mode
temp1<-aggregate(TripDistance~Mode,FUN=sum,data=place_23O)
## Create  percentage
temp1$Percent<-temp1$TripDistance*100/sum(temp1$TripDistance)
## Names
names(temp1)
...
```{r hc-25,cache=TRUE}
## Create weighted frequency
temp<-place_23O$NEXPTCFPERWGT*place_23O$TripDistance
temp2<-aggregate(temp~Mode,FUN=sum,data=place_23O)
## Create weighted percentage
temp2$Percent<-temp2$temp*100/sum(temp2$temp)
## Names
...
temp3 <- temp3[,c(1,2,4,3,5)]
temp3
temp3$avg_uw<-temp3$TDis_UW/Mode_23O$Ob_UW
temp3
temp3$avg_w<-temp3$TDis_W/Mode_23O$Ob_W
temp3
temp3[,1]<-as.character(temp3[,1])
temp3<- rbind(temp3, c("Total", colSums(temp3[,2:7],na.rm = TRUE)))
temp3
...
And lastly for the Humboldt origins, here is the drive alone modal split.  As before, 
we subset the data based on `TOTTR` (Total travelers) and `MODE={5, 6, 7, 8, 10}`
(the driver of a car, truck, or motorcycle) 

```{r hc-27,cache=TRUE} 
place_23dO<-subset(place_23O,(TOTTR==1 & (MODE==5|MODE==6|MODE==7|MODE==8|MODE==10)))
head(place_23dO)
```
...
```{r hc-28,cache=TRUE}
## Create frequency for different mode category for drive alone
temp1<-as.data.frame(table(place_23dO$Mode))
## Create percentage of frequency
temp1$Percent<-temp1$Freq*100/sum(temp1$Freq)
## Names 
names(temp1)
...
```{r hc-29,cache=TRUE}
## Create weighted frequency
temp2<-aggregate(NEXPTCFPERWGT~Mode,FUN=sum,data=place_23dO)
## Create weighted percentage
temp2$Percent<-temp2$NEXPTCFPERWGT*100/sum(temp2$NEXPTCFPERWGT)
## Names
names(temp2)
...
temp3
## Save as csv file
write.csv(temp3,file="Mode_23Odrialn.csv",row.names=FALSE)
rm(temp1)
rm(temp2)
rm(temp3)
```
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('###','Trip Purposes from Humboldt County')
```


We start with trips from Humboldt County, subsetting based open the origin county `OCTFIP=23`.

```{r hc-33, cache=TRUE}
# Now we are interested in trips from Humboldt county (OCTFIP=23) 
# Create a subset for trips from Humboldt County 
act_23O<-subset(act_place,OCTFIP==23)
names(act_23O)
head(act_23O)
```

Next, we compute the observations for trip purpose for trips from Humboldt based on new weights.

```{r hc-34,cache=TRUE}
## Generate frequency for trip Mode
temp1<-as.data.frame(table(act_23O$Actpur))
## Create percentage of frequency
temp1$Percent<-temp1$Freq*100/sum(temp1$Freq)
## Names 
names(temp1)
...
```{r hc-35,cache=TRUE}
## Create weighted frequency
temp2<-aggregate(NEXPTCFPERWGT~Actpur,FUN=sum,data=act_23O)
## Create weighted percentage
temp2$Percent<-temp2$NEXPTCFPERWGT*100/sum(temp2$NEXPTCFPERWGT)
## Names
names(temp2)
...
temp3
## Save as csv file
write.csv(temp3,file="Actpur_23O.csv",row.names=FALSE)
rm(temp1)
rm(temp2)
rm(temp3)
```
...
The last sub question wanted work trips by Mode for Humboldt County.  


```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('###','Work Trips Originating in Humboldt County')
```


First, we reuse the merged activity table from above for trips activities (and associated trips) originating in Humboldt 
County, and subset it for work trips only.

```{r hc-37}
# Create a subset for work trips
actw_23O<-subset(act_23O,APURP==9)
```
...
```{r hc-38,cache=TRUE}
## Generate frequency for trip Mode
temp1<-as.data.frame(table(actw_23O$Mode))
## Create percentage of frequency
temp1$Percent<-temp1$Freq*100/sum(temp1$Freq)
## Names 
names(temp1)
...
Given those, we can expand them with the weights for Humboldt.

```{r hc-39,cache=TRUE}
## Create weighted frequency
temp2<-aggregate(NEXPTCFPERWGT~Mode,FUN=sum,data=actw_23O)
## Create weighted percentage
temp2$Percent<-temp2$NEXPTCFPERWGT*100/sum(temp2$NEXPTCFPERWGT)
## Names
names(temp2)
...
temp3
## Save as csv file
write.csv(temp3,file="Mode_wact23O.csv",row.names=FALSE)
rm(temp1)
rm(temp2)
rm(temp3)
```
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('###','Work Trips Ending in Humboldt County')
```


Now we want to find trips ending in Humboldt county `(CTFIP=23)`.  Again, we subset
the place table and adjust the weights to be Humboldt weights if the origins are in
Humboldt, and to use statewide weights if the origins are outside Humboldt.


```{r hc-41,cache=TRUE}
# Create a subset for trips from Humboldt County 
act_23D<-subset(act_place,CTFIP==23)
names(act_23D)
head(act_23D)
# Change the weight for non Humboldt county origin (Assign state wide weight for non Humboldt origin)
act_23D$NTCFW <- ifelse((act_23D$OCTFIP==23), act_23D$NEXPTCFPERWGT, act_23D$EXPTCFPERWGT.y)
names(act_23D)
head(act_23D)
```
...
```{r hc-42}
# Generate observations by trip purpose from Humboldt based on new weights
## Generate frequency for trip Mode
temp1<-as.data.frame(table(act_23D$Actpur))
## Create percentage of frequency
temp1$Percent<-temp1$Freq*100/sum(temp1$Freq)
## Names 
names(temp1)
...
```{r hc-43,cache=TRUE}
## Create weighted frequency
temp2<-aggregate(NTCFW~Actpur,FUN=sum,data=act_23D)
## Create weighted percentage
temp2$Percent<-temp2$NTCFW*100/sum(temp2$NTCFW)
## Names
names(temp2)
...
temp3
## Save as csv file
write.csv(temp3,file="Actpur_23D.csv",row.names=FALSE)
rm(temp1)
rm(temp2)
rm(temp3)
```
...
# Create a subset for work trips
actw_23D<-subset(act_23D,APURP==9)
## Generate frequency for trip Mode
temp1<-as.data.frame(table(actw_23D$Mode))
## Create percentage of frequency
temp1$Percent<-temp1$Freq*100/sum(temp1$Freq)
## Names 
names(temp1)
...
```{r hc-46,cache=TRUE}
## Create weighted frequency
temp2<-aggregate(NTCFW~Mode,FUN=sum,data=actw_23D)
## Create weighted percentage
temp2$Percent<-temp2$NTCFW*100/sum(temp2$NTCFW)
## Names
names(temp2)
...
```{r hc-47,cache=TRUE}
## Save as csv file
write.csv(temp3,file="Mode_wact23D.csv",row.names=FALSE)
rm(temp1)
rm(temp2)
rm(temp3)
```
