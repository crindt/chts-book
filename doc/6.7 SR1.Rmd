...
98 GPS respondents within 1 km on each side of the SR1 (San Francisco
Golden Gate Bridge to Pacifica). We generated tables by mode for the
following: total number of trips, total trip distance, average trip
distance, total trip duration, average trip duration.  We also
generated tables for the total number of trips by trip purpose and for
work trips by mode. Here, we actually generated tables for total
number of trips by activity purpose since the trip purpose is given in
the database as activity purpose and there is more than one activity
in many single trips. We considered each activity as a trip. We only
considered the purpose code (APURP) 9 for work trips.  We generated
every table for both weighted and unweighted results. We used county
wide weights to generate weighted results.
...
To answer the questions we need the place file with new county wide
person weight that we computed in the section on
...
'csv' file. Now we need to import that file.
...
trips for SR1 corridor, by mode.
...
We compute the observations for trip purpose for trips of SR1 corridor
