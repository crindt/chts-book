...
This section explains how to generate summary statistics for the 2010-2012 California Household Travel Survey (2012 CHTS) and how to compare them with common variables from the 2000-2001 California Household Travel Survey (2001 CHTS). The 2012 CHTS is comprised of six main datasets and four additional datasets with location data (x-y coordinates); the latter were merged with the corresponding main datasets to check for compatibility. Merging did not create any problem, and resulted only in two minor discrepancies. First, when we merged the person-level dataset with the corresponding x-y coordinates dataset, one observation did not match in the person dataset. Second, 5 observations did not match when we merged the place dataset with the corresponding x-y coordinates dataset. We checked each variable from all datasets. To do so, we generated simple summary statistics for all variables plus boxplots and histograms for selected variables.

Overall, data quality is good.  Key statistics of variables and graphical checks did not reveal any major flaws. We also did not find major departures in the summary statistics compared to the 2001 CHTS data.

The merged household dataset for the 2012 CHTS has 42,431 observations and 83 variables. Summary statistics, boxplots and histograms for selected household variables suggest a few minor problems. First, the maximum value of HTRIPS ("Number of household trips on travel day") seems high (it is 99 for the 2012 CHTS versus 54 for the 2001 CHTS). Second, the number of observations for HHNOVs ("Reasons for not possessing a vehicle") and for the number of households without a vehicle should be equal but a cross-check shows that there are 649 missing values in HHNOVs. We also found that there are 25 cases for LDPER1 ("HH members who completed their long distance travel log") where long distance logs should have been completed but are not available.

After the household dataset, we analyze person variables. The merged person dataset has 149 variables and 109,113 observations.  Two observations have data for AGEB ("Age if refused") even though we understand they were not supposed to.  Moreover, TOLLF ("Did you use a toll?") and SLOC ("Home school") have 221 and 18 missing values respectively. A few other variables have missing values (number of values in parenthesis), including "Relationship to head of household" (1), "Gender" (1), "Hispanic or Latino" (1), "Ethnicity or race" (6), "Nativity" (1), "Toll pass" (1), "Car sharing" (1), "Employed" (1), "Disability type" (4), "Transit trips used in the past week" (1), "Walk in the last week" (1), "Online school" (1), "Pre-school location" (2), and "Level of education completed" (1). Summary statistics are presented in Table 2, which is followed by graphical illustrations of selected variables.

We then consider vehicle data. The vehicle dataset has 79,011 observations and 36 variables. Apart from missing values, there is no problem with this dataset: there are 49 missing values in VEHTRN ("Vehicle transmission"), VEHDRT ("Power train, other") and VEHCYL ("Cylinders"). Moreover, FUELT1 ("Fuel type 1") has 15 missing values, and finally "Vehicle model" and "Series" are missing 1 and 2 values respectively. 
...
Finally,  we analyze long distance travel variables. The merged dataset has 68,193 observations and 59 variables. Since there are no long distance data in the 2001 CHTS, comparison is not possible. We note 625 missing values for LDOST ("Origin State") and 809 missing values for LDDST ("Destination state"). Both LDPNM ("Origin place name") and LDOCTFIP ("Long distance trip origin location county FIP Code 2010") have one missing value. We find no other concern with long distance variables.
...
We first import the household data and corresponding look up data (that contain x-y coordinates). Before doing this, we need to save the household and look up file manually in 'csv' format and keep them in our desired location. We import those files from that location and merge them.
...
#Import the lookup variable csv file and rename it (it contains x, y coordinate data)
...
We have a merged file now, but we need to clean it by deleting duplicates
...
Now we have a new merged file that we use for diagnostics checks of the household data. We check variables one by one starting with the sample number.

```{r diag-2, cache=FALSE}
# Attach the file
attach(nhhdata)
```
...
#### RIBUS: Transit use at least once a week (1=yes;2=No;8=DK;9=RF)


```{r diag-15, cache=FALSE}
attach(nhhdata)
```
...
sd(RIBUS)
# Boxplot
boxplot(RIBUS[RIBUS<8],xlab=c("Transit use at least once a week"),col=c("gold"))
# Histogram
...
We also know that there should be no observation in O_own variable when OWN is not 7. Let us check it:
...
We know from the CHTS data that there should be previous address information if the TEN variable is greater than 6. Let us check:
...
We check the number of households with a household size greater >5 and the mean household size when household trips >30:
...
Here, we check observations with a high number of household trips:
...
There are eight variables for HHNOVs variables (reason of no possession of vehicle). Let us go through them:
...
We also check whether there is any missing value in HHNOV variables. The number of observations for HHNOV variables should equal the number of households without vehicles. Let us check: 
...
#### VEHOP: Number of operational household vehicles (Range:0-15;98=DK;99=RF)
...
hist(VEHOP,xlab=c("Number of operational household vehicles"),col=c("sky blue"))
boxplot(VEHOP,xlab=c("VEHOP"))
```



#### VEHPR: Number of vehicles with power outlets (Range:0-15;98=DK;99=RF)
...
hist(VEHPR,xlab=c("Number of vehicles with power outlets"),col=c("lawngreen"))
```



#### VEDTE:Number of newer vehicles with power outlets (Rangle:0-15)
...
hist(VEDTE,xlab=c("Number of newer vehicles with power outlets"),col=c("maroon"))
### Check whether there is any VEDTE observation without VEHPR>0 and GFLAG=3 (supposed to be zero))
...
The long distance flag should be zero when no trip is reported for long distance. Let us check it:
...
We are done with our diagnostic checks of the household data. Now we need to detach the household file.
...
We first save the person data and the corresponding look up data (that contains x-y coordinates) as a 'csv' file. We then import those two files and merge them as follows: 

```{r diag-63, cache=TRUE}
#Import the person level variables csv file and rename the file
#persondata <- read.csv("c:/Users/user/Documents/Suman_research/Caltrans Project/Data/deliv_per.csv", header=TRUE)
persondata <- chts_per

#Import the lookup variable csv file and rename the file (it containing x, y coordinate data)
...
There are a few duplicate variables that need to be deleted from the new data file.
...
Now we have a new merged file that we use for diagnostics checks of the person level data. First, we need to attach this file:

```{r diag-65, cache=FALSE}
# Attach the file
attach(npersondata)
```
...
#### RELAT: Relationship to head of household; variable values (1 to 9;99=RF)
...
Based on final report and appendix of CHTS report there should be no observation where AGE is not equal to 999 or 998. Let us check it:
...
Here we check variables for different types of transit pass


```{r diag-78, cache=FALSE}
attach(npersondata)
```
...
Let us now check more transit pass variables:
...
Now, let us generate a pie chart for WDAY1:
...
#### HOURS: Hours worked per week (Range:1-150;998=DK;999=RF)
...
Let us generate different tables with hours to cross check the employment data:
...
#### WDAYS2: Days at secondary work (RANGE:1-7;8=DK;9=RF)
...
Here, we generate a pie chart for fare unit (SUBUNT).
...
#### SCHOL: School grade level (1 to 8; 97=others;98=DK;99=RF)
...
Let us now generate a pie chart for the SCHOL variable
...
#### O_SCHOL: School grade level, other
...
Let us now analyze person trips
...
There are 10 variables related to toll road. Let us check them:
...
Let us consider work location variables:
...
Now check variables related to school location:
...
We have checked all the variables of the person dataset. Now let us detach the person data file:
...
We are now ready to check the vehicle data variables. First we import the vehicle file (a csv file), check its dimension, and see the variable names. 


```{r diag-144, cache=TRUE}
# Import the vehicle variables csv file and rename it
...
Now, we attach the vehicle dataset to check its variables one by one. 

```{r diag-145, cache=FALSE}
# Attach the file
attach(veh_data)
```
...
Now, let us start checking variables. 
...
Let us generate cross tabulations for the YEAR and MAKE variables:
...
Let us generate a pie chart for the FUELT1 variable:
...
Let us check the other FUEL variables:
...
Now, let us look at vehicle ownership variables:
...
Let us have a look at vehicle use variables.
...
We are done checking all the vehicle data variables. Let us detach the vehicle data file:
...
Now we have a new place datafile that contains location variables (x-y coordinates). We use this file for checking all the place level variables. Let us first attach this file:


```{r diag-184, cache=FALSE}
# Attach the file
attach(nplace_data)
```
...
There are five variables for person number on trip. Let us look at them:
...
#### PRKMIN: Time walking from parking to destination (Range:0-60; 98=DK;99=RF)
...
Now consider various activity variables
...
Let us check the origin and destination location variables.
...
Let us also check the trip location variables
...
We are done checking place level variables so we detach the place level file:
...
Let us now check activity data variables. First we import the activity csv file, check its dimension, and list variable names:
...
Before starting to check variables, let us attach the file:

```{r diag-229, cache=FALSE}
# Attach the file
attach(activity_data)
```

Sort the data based no sample number:
...
Now we are ready to start checking variables one by one. 
...
#### ACTRG: Number of people from the same religious/social organization
...
Now check the start and end times of activities:
...
And now have a look at weights:


#### PERWGT: Final Person weight


```{r diag-244, cache=TRUE}
## PERWGT
head(PERWGT,10)
summary(PERWGT)
boxplot(PERWGT,xlab=c("PERWGT"))
...
We are done with activity data varaibles so let us detach the file:
...
Finally, let us process the long distance dataset. As before, we import the long distance csv file, rename it, load the file with location information, and merge them.


```{r diag-251, cache=TRUE}
#Import the long distance  variables csv file and rename it
#LD_data <- read.csv("c:/Users/user/Documents/Suman_research/Caltrans Project/Data/Deliv_LD.csv", header=TRUE)
LD_data <- chts_ld

#Import the lookup variable csv file and rename it (containing x, y coordinate data)
...
Now attach the file and sort it based on sample number:


```{r diag-253, cache=FALSE}
# Attach the file
attach(nLD_data)
...
Let us now check variables related to trip origin location:


```{r diag-256, cache=FALSE}
attach(nLD_data)
...
Now consider variables related to destination location:
...
There are eight variables related with persons on long distance trip. Let us examine them:
...
Check variables related to departure time, place, and modes
...
Now examine long distance origin and destination location variables:
...
We are done checking long distance variables so we can detach the file:
