...
Now we have a new merged data frame that we use for diagnostics checks
...
variable meaning, for example, we can reference the sample number as
`SAMPN` rather than `nhhdata$SAMPN`.  This isn't completely necessary,
but makes the code look a little cleaner.
...
To look at the person data, we once again will rely on the R packages
...
There are a few duplicate variables that need to be deleted from the new data data frame.
...
Now we have a new merged data frame that we use for diagnostics checks of the person level data. First, we need to attach this data frame:
...
Based on final report and appendix of CHTS report there should be no
...
```{r diag-summary-per-WTRACT,echo=FALSE,cache=TRUE,results='asis'}
############################################################ NESTABLE SECTION
SEC("###","WTRACT: Work location census tract")
```
...
We have checked all the variables of the person dataset. Now let us detach the person data frame:
...
We are now ready to check the vehicle data variables. First we load the vehicle data frame, check its dimension, and see the variable names. 
