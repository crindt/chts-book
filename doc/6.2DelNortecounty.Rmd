...
new county wide weights of the Del Norte County to generate weighted
...
Now, we first focus on trips ending in Del Norte County.  Here we use
different weights depending on the origin of the trip.  Specifically,
when the origin is Del Norte County, we use the county-specific
weights.  When the origin is outside Del Norte County, we use the
statewide weights.
...
So `place_15$NTCFW` now contains the trips ending in Del Norte county
with appropriate expansion weights appended.  Now we can generate
observations for modal split for trips to Del Norte based on these
weights.
...
Now we perform a similar computation for trips *from* Del Norte
...
Now we generate the mode frequency table for all trips from Del Norte
County.
...
Here is the trip distance calculation for all trips from Del Norte, by
Mode.
...
And lastly for the Del Norte origins, here is the drive alone modal
...
We start with trips from Del Norte County, subsetting based open the
...
Next, we compute the observations for trip purpose for trips from Del
Norte based on new weights.
...
Now, we can determine the observations of each Mode used for work
trips.
...
Now we want to find trips ending in Del Norte county `(CTFIP=15)`.
Again, we subset the place table and adjust the weights to be Del
Norte weights if the origins are in Del Norte, and to use statewide
weights if the origins are outside Del Norte.
