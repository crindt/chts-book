...
results we first generated specific person-level weights for Del Norte
County. We then appended these weights to the place-level and
...
* Hispanic Status (Hispanic, Non-Hispanic)---Del Norte County
  distribution.

* Ethnicity (White, African American, Asian, Other)---Del Norte County
  distribution.

* Age (less than 20 years, 20--24 years, 25--34 years, 35--54
  years, 55--64 years, 65 years or older)---Del Norte County
  distribution.

* Employment Status (Part-time or full-time Employed, Not-employed) -
  Del Norte County distribution Gender (Male, Female)---Del Norte
  County distribution
...
represent the total population of Del Norte County, we performed some
random checks for different variables; results showed that the new
weights align well with the population of Del Norte County. We then
calculated the final new trip weights and used them to generate the
weighted results that follow.

We generated tables by mode for the following: total number of trips,
...
To calculate the drive alone numbers we extracted data for total
travelers \ifmydocumentation(`TOTTR=1`)\fi and drivers of cars,
trucks, or motorcycles \ifmydocumentation(`MODE=5, 6, 7, 8,
...
table for both trips originating in and ending in Del Norte County. We
used the new weights for Del Norte County to generate weighted results
for trips that originated in Del Norte County and statewide weights to
generate weighted results for trips that originated outside of Del
Norte County. R code (RStudio) to generate weights and the requested
tables is given \ifmydocumentation below\else in the appendix DVD\fi.
