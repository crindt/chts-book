...
```{r table-a_1-results,results='asis'}
cat(res.a.1.str)
```
...
The next step is to compute the category marginals.  We create a
convenience function that we'll reuse for each county (and the state).
...
Now, `data2` has the linked trips for each weekday person trip.  We
can simply compute the mean for each combination. 
...
lt.dat <- chts_act %>% select(SAMPN,PERNO,PLANO,ACTNO,APURP) %>% filter(APURP==17) %>%
  ...
  select(linkedtripno) %>%
  left_join(lt.clean,by=c('linkedtripno'))
