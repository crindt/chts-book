...
```{r rt-setup-0,echo=FALSE,cache=FALSE,warn=FALSE}
if ( !exists("rd") ) { rd <<- gsub("(.*?chts-book[^/]*).*$","\\1",getwd()) } 
source(paste(rd,'R/setup-hooks.R',sep="/"))
# remove chts data from namespace
null<-lapply(
...
# Using R to Generate Summary Tables

In this section, we describe the creation of a set of summary tables
that provide statistics about household travel behavior as derived
from the CHTS dataset.  The tables generated were requested by
Caltrans staff.  The table referencing (table A, table B, etc.) is
defined in order to correspond with similar tables produced for the
2001 dataset.  This is true for all tables described herein except for
table 50 and greater, which represent requests from Caltrans staff
that don't have an analogue in the 2001 report.

In the sections below, we provide the `R` code for generating each
table as well as some discussion of the general methods used.
...
fipsctyca <- read.csv(pd("fips-county.csv"),
                      header=TRUE,
...
Weights used in the calculations of the following tables were
generated separately and included in the `chts2011wgt` package.  See
the documentation of that package for details.  Here we load in
various weights used in the calculations below, including weekday
weights and county weights (and county weekday weights).

```{r rt-load-weights}
library(chts2011wgt)
```

## Table A: Key 2011-2012 Travel Survey Data for California
...
weekday.  We loaded these weights earlier.
...
lohi <- as.integer(
  gsub("[$,]","",
  (unlist(strsplit(as.character(catvals[[1]])," to ")))))


# Determine parameters for pareto interpolation
a <- lohi[1]  # income value at the lower limit of the cat. containing the median
b <- lohi[2]  # income value at the upper limit of the cat. containing the median
...
We also write the data to a CSV file for use with external programs.
...
```{r rt-table-a_2b,results='asis',cache=TRUE}
...
cat(res.a.2.str)
res[["res.a.1"]] <- list()
res[["res.a.1"]]$title  <- "TABLE A: Household Vehicle Availability"
res[["res.a.1"]]$data <- res.a.1
res[["res.a.1"]]$tabtxt <- res.a.1.str
res[["res.a.2"]] <- list()
res[["res.a.2"]]$title  <- "TABLE A: Household Vehicle Availability (cont)"
res[["res.a.2"]]$data <- res.a.2
res[["res.a.2"]]$tabtxt <- res.a.2.str
```
...
```{r rt-results-b-0,results='asis',cache=TRUE,echo=FALSE}
cat(b.0.str)
```
...
```{r rt-table-b-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.b.str)
res[["res.b"]] <- list()
...
## Table C - Travel Mode Distribution
...
```{r rt-table-c-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.c.str)
res[["res.c"]] <- list()
...
## Table D: Weekday Commute Mode Share Information
...
```{r rt-table-d-results,results='asis',echo=FALSE,cache=TRUE}
cat(res[['d.str']])
res[["res.d"]] <- list()
...
```{r rt-table-1-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.1.str)
res[["res.1"]] <- list()
...
lic.ca <- chts_hh %>% summarise(
  County='California',  
  Licensed=round(wtd.mean(HHLIC,weights=EXPHHWGT,na.rm=TRUE),1)
  )
```
...
```{r rt-table-5-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.5.str)
res[["res.5"]] <- list()
...
emp.ca <- chts_hh %>% summarise(
  County='California',
  Employed=round(wtd.mean(HHEMP,weights=EXPHHWGT,na.rm=TRUE),1)
  )
```
...
```{r rt-table-6-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.6.str)
res[["res.6"]] <- list()
...
```
```{r rt-table-9-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.9.1.str)
```
  ...
  paste(  # paste will concatenate the results of the lapply function
    lapply(
      ...
      function(cty) {
        ...
        if(exists("res.9.2")) {
          res.9.2 <<- rbind(res.9.2,
                            cbind(data.frame(County=cty),tabtot))
        } else {
            res.9.2 <<- cbind(data.frame(County=cty),tabtot)
        }
        return(
          pandoc.table.return(
            tabtot,
...
```
```{r rt-t9-2-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.9.2.str)
res.9.str <- paste(res.9.1.str,res.9.2.str,sep="\n\n\n")
res[["res.9"]] <- list()
res[["res.9"]]$title  <- "TABLE 9: Weekday Trip Type Distribution"
res[["res.9"]]$data <- rbind(cbind(data.frame(County="California"),res.9.1),
                             res.9.2)
...
```{r rt-t10a-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.10a.str) 
res[["res.10a"]] <- list()
...
```{r rt-t11-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.11.str) 
```
...
```{r rt-t13a-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.13.str)
```
...
```{r rt-table15-output,results='asis',cache=TRUE}
cat(res.15.str)
```
...
Now, `data` has the linked trips for each weekday driver trip.  We can
simply compute the mean for each combination.
...
```{r rt-table15a-output,results='asis',cache=TRUE}
cat(res.15a.str)
```
...
```
```{r rt-table-17a-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.17a.st.str)
```
  ...
  paste(
    lapply(
      ...
      function(icty) {
        ...
        if( exists("res.17a.cty") ) {
          res.17a.cty <<- rbind(res.17a.cty,
                                cbind(data.frame(County=cty),
                                      fftab.all))
        } else {
          res.17a.cty <<- cbind(data.frame(County=cty),fftab.all)
        }
...
```{r rt-t17a-cty-results,echo=FALSE,results='asis',cache=TRUE}
cat(res.17a.cty.str)
res[["res.17a"]] <- list()
res[["res.17a"]]$title  <- "TABLE 17a: Weekday Linked and Unlinked Trips Distributed by Particular Travel Mode and Group"
res[["res.17a"]]$data <- rbind(cbind(County="California",res.17a.st),res.17a.cty)
res[["res.17a"]]$tabtxt <- paste(res.17a.st.str,res.17a.cty.str,sep="\n\n\n")
```


## Table 17B and 17C: Weekday Commuters' Directional Travel Mode Split
    ...
    lapply(
      ...
      function(cty) {
        ...
        fftab.all <- merge(fftab.allday,fftab.am,by=c('MODE.GROUP')) %>%
          ...
          mutate(County=cty)
        if ( !exists("res.17bc") ) {
          res.17bc <<- cbind(data.frame(County=cty),
                             fftab.all %>% arrange(order) %>% select(-order,-County))
        } else {
          res.17bc <<- rbind(res.17bc,
                             cbind(data.frame(County=cty),
                                   fftab.all %>% arrange(order) %>% select(-order,-County)))
        }
  ...
  paste(
    lapply(
      ...
      function(cty) {
        ...
        if ( is.null(res.22.data) ) {
          res.22.data <<- cbind(data.frame(County=cty),res.22)
          } else {
            res.22.data <<- rbind(res.22.data,cbind(data.frame(County=cty),res.22))
            }
        return(
          pandoc.table.return(
            res.22,
...
```
```{r rt-table-22-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.22.str) 
res[["res.22"]] <- NULL
res[["res.22"]] <- list()
res[["res.22"]]$title  <- "TABLE 22: Vehicles in Use on Average Weekday Distributed by Age of Vehicle and by County"
res[["res.22"]]$data <- res.22.data
res[["res.22"]]$tabtxt <- res.22.str
```


## TABLE 22a: Vehicle Trips, Person Trips by County by Mode and Purpose
...
## TABLE 50: Mode Share for Short Trips by County
...
```
```{r rt-table-50-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.50.str)
res[["res.50"]] <- list()
...
## TABLE 51: Mode Share for Millennials Versus Older Age Groups
...
```
```{r rt-table-51-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.51.str)
res[["res.51"]] <- list()
...
## TABLE 52: Statewide Commute trips by Mode and Distance Ranges
...
```
```{r rt-table-52-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.52.str)
res[["res.52"]] <- list()
...
## TABLE 53: Statewide School Trips by Mode and Distance Ranges
...
```
```{r rt-table-53-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.53.str)
res[["res.53"]] <- list()
...
## TABLE 54: Daily Travel Statistics by Weekday vs Weekend

We will compute a number of driver- and person-specific daily travel
statistics partitioned by weekday/weekend as and spatially (by
County).  For the purposes of this table, we assume the spatial
partitioning is by household location.  All statistics are computed
using the linked-trip table.  We start by creating a data frame
holding all linked-trips recorded for drivers then add the weights we
computed earlier for each county and weekday/weekend days.

```{r rt-t54-1}
#Daily Travel Statistics
##Vehicle Trips per Driver

# use lt.wd.trips.veh, but expand to county
trips.veh <- 
  lt.clean %>% 
  filter(ltmode==5) %>% # driver trips only
  left_join(chts_hh %>% select(SAMPN,CTFIP,DOW) %>% 
              transmute(SAMPN=SAMPN,
                        hCTFIP=as.integer(CTFIP)%%1000,
                        WD=ifelse(DOW<6,1,2)),
            by=c('SAMPN')
            ) 

# people w/ wd+county weights
trips.wd.cty.veh <-
  trips.veh %>%
  left_join(chts_per.wgt.wd.cty,
            by=c('SAMPN','PERNO')
            )
```

Since we also want the state-wide statistics, we duplicate all county
trips and overwrite their county codes (`HCTFIP=0`) and weights to be
state-wide weekend/weekday weights.  By combining them into a single
data frame, we can streamline code below.


```{r rt-t54-2}
trips.wd.cty.veh <- 
  rbind(trips.wd.cty.veh,
        # attach duplicates for California as a whole
        trips.wd.cty.veh %>% 
          left_join(chts_per.wgt.wd %>% select(SAMPN,PERNO,NEXPPERWDWGT),
                    by=c('SAMPN','PERNO')
                    ) %>%
          mutate(
            hCTFIP=0,
            NEXPPERWDCTYWGT=NEXPPERWDWGT) %>%
          select(-NEXPPERWDWGT)) %>%
  mutate(NEXPTCFPERWDCTYWGT=NEXPPERWDCTYWGT*ltTCF)        # compute TCF weights

veh.trips.wd.cty <- trips.wd.cty.veh %>%
  group_by(hCTFIP,WD) %>%
  summarise(vtrips=sum(NEXPPERWDCTYWGT))
```

Now, we turn to computing vehicle trips per driver.  Given the other
content of the requested table (e.g., average trip length), we assume
this should be vehicle trips per "active" driver.  Thus, we compute
the number of drivers by totaling the unique drivers making trips in
the survey by county and day (recall that we've created a duplicate
set of data representing statewide activity).

```{r rt-t54-3}
# same number of drivers in county on wd/we
# we compute drivers as total number of licensed drivers
drivers.cty <- trips.wd.cty.veh %>%
  # group by SAMPN, PERNO to get unique drivers
  group_by(hCTFIP,WD,SAMPN,PERNO) %>%  

  # summarise to get one row (with a weight) for each driver
  summarise(trips=n(),wgt=max(NEXPPERWDCTYWGT)) %>%

  # now group by county/day combo
  group_by(hCTFIP,WD) %>%

  # and sum the weights to get a weighted count of drivers by
  # county/day comno
  summarise(drivers=sum(wgt))
```            

Next, we compute the vehicle trips per driver by combining trip and
driver frames.  We'll attach all computations to the `res.54.all` data
frame.

```{r rt-t54-4}
vtpd.cty.day <-
  veh.trips.wd.cty %>%
  left_join(drivers.cty,by=c('hCTFIP','WD'))  %>%
  mutate('Vehicle Trips per Driver'=vtrips/drivers) %>% 
  select(-vtrips,-drivers)
#  mutate('Day'=ifelse(WD==1,'Weekday','Weekend')) %>% select(-WD)

res.54.all <- vtpd.cty.day
```

Now we turn to computing the fraction of work and non-work trips in
this sample.  We use the linked-trip `TripType` variable to identify
any work-related trips then compute the fractions accordining.
...
wtrips <- trips.wd.cty.veh %>%
  filter(TripType=='HW' | TripType=='WO' | TripType=='WW') %>%
  group_by(hCTFIP,WD) %>%
  summarise(vtrips.wrk=sum(NEXPPERWDCTYWGT))

trippct <- veh.trips.wd.cty %>%
  left_join(wtrips,by=c('hCTFIP','WD')) %>% 
  mutate('Pct Work Trips'=vtrips.wrk/vtrips*100,
         'Pct Non-Work Trips'=(1-vtrips.wrk/vtrips)*100) %>%
  select(-vtrips,-vtrips.wrk)

res.54.all <-
  res.54.all %>%
  left_join(trippct,by=c('hCTFIP','WD'))
```

To compute VMT per driver, we'll reuse the `drivers` data frame from
above, but need to count the VMT for each county/day by computing a
weighted sum of the trip distances, which we again repeat for the
statewide totals too.
...
vmt.cty <- trips.veh %>%
  left_join(chts_per.wgt.wd.cty ,by=c('SAMPN','PERNO')) %>%
  group_by(WD,hCTFIP) %>%
  summarise(vmt=sum(tripdist*NEXPPERWDCTYWGT*ltTCF,na.rm=TRUE))
vmt.ca <- trips.veh %>% 
    left_join(chts_per.wgt.wd %>% select(-WD),by=c('SAMPN','PERNO')) %>%
    group_by(WD) %>%
    summarise(vmt=sum(tripdist*NEXPPERWDWGT*ltTCF,na.rm=TRUE)) %>%
    transmute(WD=WD,hCTFIP=0,vmt)
...
vmt.per.driver <- vmt %>%
  left_join(drivers.cty,by=c('hCTFIP','WD')) %>%
  mutate(vmt.per.driver=vmt/drivers) %>%
  select(-vmt,-drivers)

res.54.all <- res.54.all %>% left_join(vmt.per.driver,by=c('hCTFIP','WD'))
```

The average vehicle trip length can by computed as a simple weighted
mean of `tripdist` for each county/day.

```{r rt-t54-7}

#Average Vehicle Trip Length

mean.vehicle.trip.length <- 
  rbind(
    trips.veh %>%
      left_join(chts_per.wgt.wd.cty,by=c('SAMPN','PERNO')) %>%
      group_by(WD,hCTFIP) %>%
      summarise(avg.len=wtd.mean(tripdist,weights=NEXPPERWDCTYWGT*ltTCF,na.rm=TRUE)),
    trips.veh %>% 
      left_join(chts_per.wgt.wd %>% select(-WD,-fpc),by='SAMPN','PERNO') %>%
      group_by(WD) %>%
      summarise(avg.len=wtd.mean(tripdist,weights=NEXPPERWDWGT*ltTCF,na.rm=TRUE)) %>%
      transmute(hCTFIP=0,WD,avg.len)
    )

res.54.all <- res.54.all %>%
  left_join(mean.vehicle.trip.length,by=c('hCTFIP','WD'))
```

The average time spent driving is also a simple weighted mean, this time of `tripdur`.

```{r rt-t54-8}
#Average Time Spent Driving (in minutes)
mean.vehicle.trip.dur <- rbind(
  trips.veh %>%
    left_join(chts_per.wgt.wd.cty,by=c('SAMPN','PERNO')) %>%
    group_by(WD,hCTFIP) %>%
    summarise(avg.dur=wtd.mean(tripdur,weights=NEXPPERWDCTYWGT*ltTCF,na.rm=TRUE)),
  trips.veh %>%
    left_join(chts_per.wgt.wd %>% select(-WD,-fpc),by=c('SAMPN','PERNO')) %>%
    group_by(WD) %>%
    summarise(avg.dur=wtd.mean(tripdur,weights=NEXPPERWDWGT*ltTCF,na.rm=TRUE)) %>%
    transmute(hCTFIP=0,WD,avg.dur))
res.54.all <- res.54.all %>%
  left_join(mean.vehicle.trip.dur,by=c('hCTFIP','WD'))

```

The next elements in the table are for person trips, using the
definitions described earlier.  As for the vehicle trip computations
above, we start by filtering the linked-trip table for person trips
and join on the household county and associated county/day weights.
...
lt.person.trips <-
  ...
  left_join(chts_hh %>%
              select(SAMPN,CTFIP) %>%
              mutate(hCTFIP=as.integer(CTFIP)%%1000),
            by=c('SAMPN')) 
...
lt.wd.cty.trips <- 
  ...
  left_join(chts_per.wgt.wd.cty,by=c('SAMPN','PERNO')) %>%              
  # add WD variable
  left_join(chts_hh.wgt.wd %>% select(SAMPN,WD),by=c('SAMPN')) %>%   
  mutate(NEXPTCFPERWDCTYWGT=NEXPPERWDCTYWGT*ltTCF)
```

Since we need some statistics per person, we again assume these
statistics should by for *traveling* persons (not total persons), and
compute total persons traveling in each county and in the state
...
persons.cty <- traveling.persons %>% 
  left_join(chts_per.wgt.wd.cty,by=c('SAMPN','PERNO')) %>%
  left_join(chts_hh %>%transmute(SAMPN,CTFIP,WD=ifelse(DOW<6,1,2)),
            by=c('SAMPN')) %>%
  mutate(hCTFIP=as.integer(CTFIP)%%1000) %>%
  group_by(hCTFIP,WD) %>%
  summarise(persons=sum(NEXPPERWDCTYWGT))

persons.st <- traveling.persons %>%
  left_join(chts_per.wgt.wd,by=c('SAMPN','PERNO')) %>%
  left_join(chts_hh %>%select(SAMPN,CTFIP),
            by=c('SAMPN')) %>%
  mutate(hCTFIP=0) %>%
  group_by(hCTFIP,WD) %>%
  summarise(persons=sum(NEXPPERWDWGT))

persons <- rbind(
  persons.st, persons.cty
  )
```

Next, we compute the weighted person-trip sums for each county/day.

```{r rt-t54-11}
person.trips.wd.cty <- 
  lt.wd.cty.trips %>% group_by(hCTFIP,WD) %>%
  summarise(trips=sum(NEXPTCFPERWDCTYWGT,na.rm=TRUE))

person.trips.wd.st <-
  lt.wd.cty.trips %>% select(-WD,-fpc) %>% 
  left_join(chts_per.wgt.wd,by=c('SAMPN','PERNO')) %>%
  group_by(WD) %>%
  summarise(trips=sum(NEXPPERWDWGT*ltTCF,na.rm=TRUE)) %>%
  transmute(hCTFIP=0,WD,trips)

person.trips.all <- rbind(
  person.trips.wd.st,
  person.trips.wd.cty
  )
```

Then, we divide the total trips by total traveling persons for each
county/day and store the result.

```{r rt-t54-12}
            
person.trips.per.person.wd.cty <-
  person.trips.all %>% 
  left_join(persons,by=c('hCTFIP','WD')) %>% 
  mutate(ptpp=trips/persons) %>%
  select(-trips,-persons)

res.54.all <- res.54.all %>%
  left_join(person.trips.per.person.wd.cty,by=c('hCTFIP','WD'))
```

Turning to the person-miles traveled per person, we compute a weighted
sum of the tripdist for each county/day and divide by traveling persons.

```{r rt-t54-13}
#PMT per Person
pmt.wd.cty <-
  lt.wd.cty.trips %>% group_by(hCTFIP,WD) %>%
  summarise(pmt=sum(NEXPTCFPERWDCTYWGT*tripdist,na.rm=TRUE))

pmt.wd.st <-
  lt.wd.cty.trips %>% select(-WD,-fpc) %>%
  left_join(chts_per.wgt.wd,by=c('SAMPN','PERNO')) %>% 
  group_by(WD) %>%
  summarise(pmt=sum(NEXPPERWDWGT*ltTCF*tripdist,na.rm=TRUE)) %>%
  transmute(hCTFIP=0,WD,pmt)
...
pmt.per.person.wd.cty <-
  pmt.wd %>%
  left_join(persons,by=c('hCTFIP','WD')) %>%
  mutate(pmtpp=pmt/persons) %>%
  select(-pmt,-persons)

res.54.all <- res.54.all %>%
  left_join(pmt.per.person.wd.cty,by=c('hCTFIP','WD'))
```

To get average person trip length, we compute the straight weighted
mean of tripdist.
...
aptl.cty <-
  ...
  group_by(hCTFIP,WD) %>%
  summarise(
    avg.person.trip.length=wtd.mean(tripdist,
                                    weights=NEXPPERWDCTYWGT*ltTCF,
                                    na.rm=TRUE))

aptl.st <- lt.wd.cty.trips %>% select(-WD,-fpc) %>%
  left_join(chts_per.wgt.wd,by=c('SAMPN','PERNO')) %>%
  group_by(WD) %>%
  summarise(avg.person.trip.length=wtd.mean(tripdist,
                                            weights=NEXPPERWDWGT*ltTCF,
                                            na.rm=TRUE)) %>%
  transmute(hCTFIP=0,WD=WD,avg.person.trip.length)

avg.person.trip.length <- rbind(
  aptl.st,
  aptl.cty)

res.54 <- res.54.all %>%
  left_join(avg.person.trip.length,by=c('hCTFIP','WD'))
```

Now we're ready to put it all together.  We start by creating a
dataframe to attach county and state names for each distinct `hCTFIP`.

```{r rt-t54-15}
fipscty <- rbind(
  fipsctyca,
  data.frame(state=c("CA"),state_ansi=c(6),county_ansi=c(0),
             county_name="California",ansi_cl=c("H1"),County="California"))
```

Then, we join that data frame and transform the table to contain
user-friendly data for use in a table.

```{r rt-t54-16}

res.54.all <- res.54 %>%
  left_join(fipscty,by=c('hCTFIP'='county_ansi')) %>%
  transmute(County,
            WD=ifelse(WD==1,"Weekday","Sat/Sun"),
            'Vehicle Trips per Driver'=`Vehicle Trips per Driver`,
            'Pct Works Trip'=`Pct Work Trips`,
            'Pct Non-Work Trips'=`Pct Non-Work Trips`,
            'VMT per Driver'=vmt.per.driver,
            'Average Trip Length'=avg.len,
            'Average Trip Duration'=avg.dur,
            'Person Trips per Person'=ptpp,
            'Person Miles per Person'=pmtpp,
            'Average Person Trip Length'=avg.person.trip.length)
```

In the next step, we generate pandoc-compatible formatted tables for
each county.  We do this by looping over the counties (and state),
filtering the main data frame for the relevant subset of data,
manipulating it to match the requested format, then outputing the
result.  The results are all concatenated into a single string, which
we store.
...
res.54.str <- 
  paste(
    lapply(
      c("California",sort(unique(res.54.all$County))),
      function(cty) {
        
        # get the data for <cty> County
        res.54<-t(res.54.all %>% 
                    ungroup() %>% 
                    filter(County==cty) %>% 
                    select(-hCTFIP,-County) ) %>% 
          as.data.frame() 
        
        # transpose the data to match the requested format
        names(res.54) <- as.vector(t(res.54['WD',]))

        # remove the "county" row (it's redundant)
        res.54 <- res.54[-1,]

        # the transpose operation made the data factors,
        # convert back to numbers (this is a little hack-y)
        res.54$Weekday <- as.numeric(as.character(res.54$Weekday))
        res.54$`Sat/Sun` <- as.numeric(as.character(res.54$`Sat/Sun`))

        # format and return the table
        return(
          pandoc.table.return(
            res.54,
            caption=paste("TABLE 54: Daily Travel Statistics by Weekday vs Weekend (",
                          cty,")",sep=""),
            split.table=Inf,split.cells=Inf,
            justify=c('left','right','right'),
            style='multiline',
...
Finally, print out the table and store the results in the table list.

```{r rt-table-54-results,results='asis',echo=FALSE,cache=TRUE}
cat(res.54.str)
res[["res.54"]] <- list()
res[["res.54"]]$title <-
  "TABLE 54: Daily Travel Statistics by Weekday vs Weekend for each County"
res[["res.54"]]$data <- res.54.all
res[["res.54"]]$tabtxt <- res.54.str
```

## TABLE 55: Selected travel data for older population groups

For this table, we use a similar approach to table 54, but limit it to
older drivers.  We're also only interested in statewide data for all
days, so we can use the standard weights.
        ...
        agecat=ifelse(
            AGE<60,"50-59",
               ifelse(
                   AGE<70,"60-69",
               ifelse(
                   AGE<80,"70-79","80 and older"))),
        gend=ifelse(
            GEND==1,"Male",
            ifelse(GEND==2,"Female","N/A")))

# Percent Licensed Drivers
over.50.lic.ft <- over.50 %>%
    mutate(gend="All") %>%
    group_by(gend,agecat,LIC) %>%
    summarise(n=sum(EXPPERWGT)) %>%
    mutate(freq.pct.drivers = round(n / sum(n)*100,1)) %>%
    filter(LIC==1)

over.50.lic.ft <-
    rbind(over.50 %>%
          mutate(gend="All",agecat="All 50 and Over") %>%
          group_by(gend,agecat,LIC) %>%
          summarise(n=sum(EXPPERWGT)) %>%
          mutate(freq.pct.drivers = round(n / sum(n)*100,1)) %>%
          filter(LIC==1),
...
over.50.lic.ft <-
    rbind(over.50.lic.ft,
          over.50 %>%
          group_by(gend,agecat,LIC) %>%
          summarise(n=sum(EXPPERWGT)) %>%
          mutate(freq.pct.drivers = round(n / sum(n)*100,1)) %>%
          filter(LIC==1 & (gend %in% c('Male','Female')))
          )
over.50.lic.ft <-
    rbind(over.50 %>%
          mutate(agecat="All 50 and Over") %>%
          group_by(gend,agecat,LIC) %>%
          summarise(n=sum(EXPPERWGT)) %>%
          mutate(freq.pct.drivers = round(n / sum(n)*100,1)) %>%
          filter(LIC==1 & (gend %in% c('Male','Female'))),
...
over.50.trips <-  over.50 %>%
    left_join(chts_pla %>% select(-EXPPERWGT),by=c('SAMPN','PERNO'))
over.50.driver.trips <- over.50.trips %>%
    filter(MODE %in% driver.trip.modes)
over.50.driver.trips.stats <- over.50.driver.trips %>%
    group_by(SAMPN,PERNO) %>%
    summarise(n=sum(ifelse(LIC==1,EXPPERWGT,0),na.rm=TRUE),
              vmt=sum(EXPTCFPERWGT*TripDistance,na.rm=TRUE)) %>%
    left_join(over.50 %>% select(SAMPN,PERNO,gend,agecat), by=c('SAMPN','PERNO'))

over.50.driver.vmt <- over.50.driver.trips.stats %>%
    mutate(gend='All') %>%
    group_by(gend,agecat) %>%
    summarise(vmt.per.d=round(sum(vmt)/sum(n),1))
    
over.50.driver.vmt <-
    rbind(over.50.driver.trips.stats %>%
          mutate(gend='All',agecat="All 50 and Over") %>%
          group_by(gend,agecat) %>%
          summarise(vmt.per.d=round(sum(vmt)/sum(n),1)),
          over.50.driver.vmt)
...
over.50.driver.vmt <-
    rbind(over.50.driver.vmt,
          over.50.driver.trips.stats %>%
          group_by(gend,agecat) %>%
          summarise(vmt.per.d=round(sum(vmt)/sum(n),1)) %>%
          filter(gend %in% c('Male','Female')))
...
over.50.hh <- over.50 %>%
    left_join(chts_hh %>% select(SAMPN,HHVEH),by=c('SAMPN'))

over.50.noveh <- over.50.hh %>%
          mutate(gend="All") %>%
          group_by(gend,agecat,HHVEH) %>%
          summarise(n=sum(EXPPERWGT)) %>%
          mutate(freq.noveh = round(n / sum(n)*100,1)) %>%
          filter(HHVEH==0)
...
over.50.noveh <-
    rbind(over.50.noveh,
          ...
          summarise(n=sum(EXPPERWGT)) %>%
          mutate(freq.noveh = round(n / sum(n)*100,1)) %>%
          filter(HHVEH==0 & (gend %in% c('Male','Female')))
          )
...
over.50.ntrips.data <- over.50 %>%
    left_join(chts_pla,by=c('SAMPN','PERNO')) %>%
    group_by(SAMPN,PERNO) %>%
    summarise(ntrip=sum(ifelse(is.na(PLANO),0,1))) %>%
    left_join(over.50 %>% select(SAMPN,PERNO,agecat,gend,EXPPERWGT),
              by=c('SAMPN','PERNO'))

over.50.ntrips <- over.50.ntrips.data %>%
          mutate(gend="All") %>%
          group_by(gend,agecat,ntrip) %>%
          summarise(n=sum(EXPPERWGT)) %>%
          mutate(freq.no.trips = round(n / sum(n)*100,1)) %>%
          filter(ntrip==0)

over.50.ntrips <-
    rbind(over.50.ntrips.data %>%
          mutate(gend="All",agecat="All 50 and Over") %>%
          group_by(gend,agecat,ntrip) %>%
          summarise(n=sum(EXPPERWGT)) %>%
          mutate(freq.no.trips = round(n / sum(n)*100,1)) %>%
...
over.50.ntrips <-
    rbind(over.50.ntrips,
          over.50.ntrips.data %>%
          group_by(gend,agecat,ntrip) %>%
          summarise(n=sum(EXPPERWGT)) %>%
          mutate(freq.no.trips = round(n / sum(n)*100,1)) %>%
          filter(ntrip==0 & (gend %in% c('Male','Female')))
          )
...
over.50.disab <- over.50 %>%
          mutate(gend="All") %>%
          group_by(gend,agecat,DISAB) %>%
          summarise(n=sum(EXPPERWGT)) %>%
          mutate(freq.disab = round(n / sum(n)*100,1)) %>%
          filter(DISAB==1)
...
over.50.disab <-
    rbind(over.50.disab,
          ...
          summarise(n=sum(EXPPERWGT)) %>%
          mutate(freq.disab = round(n / sum(n)*100,1)) %>%
          filter(DISAB==1 & (gend %in% c('Male','Female')))
          )
...
```{r rt-t55-join}
res.55.all <-
    over.50.lic.ft %>% select(gend,agecat,freq.pct.drivers) %>%
    left_join(over.50.driver.vmt %>%
              select(gend,agecat,vmt.per.d), by=c('gend','agecat')) %>%
    left_join(over.50.noveh %>%
              select(gend,agecat,freq.noveh), by=c('gend','agecat')) %>%
    left_join(over.50.ntrips %>%
              select(gend,agecat,freq.no.trips), by=c('gend','agecat')) %>%
    left_join(over.50.disab %>%
              select(gend,agecat,freq.disab), by=c('gend','agecat')) %>%
    select(Gender=gend,
           'Age Group'=agecat,
           'Percent Drivers'=freq.pct.drivers,
           'Vehicle Miles per Driver'=vmt.per.d,
           'Percent with No Vehicles Available'=freq.noveh,
           'Percent Who Did Not Travel on Travel Day'=freq.no.trips,
           'Percent Disabled'=freq.disab)
```
...
res.55.str <-
    pandoc.table.return(
        res.55.all,
        caption="TABLE 55: Selected travel data for older population groups",
        split.table=Inf,
        split.cells=c(5,20,rep(10,5)),
        justify=c('left','left','right','right','right','right','right'),
        style='multiline',
        round=1,
        big.mark = ",",
        missing='&nsbp;',
        emphasize.strong.rows=which(res.55.all$`Age Group`=='All 50 and Over',arr.ind=TRUE)
        
    )
```

```{r rt-t-55-results,results='asis',echo=FALSE,cache=FALSE}
cat(res.55.str)
res[["res.55"]] <- list()
res[["res.55"]]$title  <- "TABLE 55: Selected travel data for older population groups"
res[["res.55"]]$data <- res.55.all
res[["res.55"]]$tabtxt <- res.55.str
```


## TABLE 56: Daily travel stats by age

For this table, we use the same logic as in TABLE 54, but instead of
weighting by county, we weight statewide and compute the statistics
for the various age categories requested.
...
trips.peeps.veh <-
  ...
  mutate(age.group=
           ifelse(
             AGE<18,"NA",
             ifelse(
               AGE<35,"Younger (Millenials) 18-34",
               ifelse(
                 AGE<50,"Young (Generation X) 35-49",
                 ifelse(
                   AGE<65,"Baby Boomers 50-64",
                   "Older 65+")))))
                      
                    
```

Next, we compute weighted vehicle trips by the age groups.

```{r rt-t56-2}
veh.trips.age.group <- trips.peeps.veh %>%
  group_by(age.group) %>%
  summarise(vtrips=sum(EXPPERWGT*ltTCF,na.rm=TRUE))
```
...
# same number of drivers in county on wd/we
# we compute drivers as total number of licensed drivers
drivers.age.group <- trips.peeps.veh %>%
  # group by SAMPN, PERNO to get unique drivers
  group_by(age.group,SAMPN,PERNO) %>%  

  # summarise to get one row (with a weight) for each driver
  summarise(trips=n(),wgt=max(EXPPERWGT)) %>%
...
Next, we compute the vehicle trips per driver by combining trip and
driver frames.  We'll attach all computations to the `res.56.all` data
frame.

```{r rt-t56-4}
vtpd.age.group <-
  veh.trips.age.group %>%
  left_join(drivers.age.group,by=c('age.group'))  %>%
  mutate('Vehicle Trips per Driver'=vtrips/drivers) %>% 
  select(-vtrips,-drivers)
#  mutate('Day'=ifelse(WD==1,'Weekday','Weekend')) %>% select(-WD)

res.56.all <- vtpd.age.group
```
...
wtrips <- trips.peeps.veh %>%
  filter(TripType=='HW' | TripType=='WO' | TripType=='WW') %>%
  group_by(age.group) %>%
  summarise(vtrips.wrk=sum(EXPPERWGT*ltTCF,na.rm=TRUE))

trippct <- veh.trips.age.group %>%
  left_join(wtrips,by=c('age.group')) %>% 
  mutate('Pct Work Trips'=vtrips.wrk/vtrips*100,
         'Pct Non-Work Trips'=(1-vtrips.wrk/vtrips)*100) %>%
  select(-vtrips,-vtrips.wrk)
...
res.56.all <-
  res.56.all %>%
  left_join(trippct,by=c('age.group'))
```

To compute VMT per driver, we'll reuse the `drivers` data frame from
above, but need to count the VMT for age group by computing a
weighted sum of the trip distances.

```{r rt-t56-6}

#VMT per Driver

vmt.age.group <- trips.peeps.veh %>%
  group_by(age.group) %>%
  summarise(vmt=sum(tripdist*EXPPERWGT*ltTCF,na.rm=TRUE))

vmt.per.driver <- vmt.age.group %>%
  left_join(drivers.age.group,by=c('age.group')) %>%
  mutate(vmt.per.driver=vmt/drivers) %>%
  select(-vmt,-drivers)

res.56.all <- res.56.all %>% left_join(vmt.per.driver,by=c('age.group'))
```

The average vehicle trip length can by computed as a simple weighted
mean of `tripdist` for each age group.
...
mean.vehicle.trip.length <- 
  trips.peeps.veh %>%
  group_by(age.group) %>%
  summarise(avg.len=wtd.mean(tripdist,weights=EXPPERWGT*ltTCF,na.rm=TRUE))

res.56.all <- res.56.all %>%
  left_join(mean.vehicle.trip.length,by=c('age.group'))
```

The average time spent driving is also a simple weighted mean, this
time of `tripdur`.

```{r rt-t56-8}
#Average Time Spent Driving (in minutes)
mean.vehicle.trip.dur <-
  trips.peeps.veh %>%
    group_by(age.group) %>%
    summarise(avg.dur=wtd.mean(tripdur,weights=EXPPERWGT*ltTCF,na.rm=TRUE))
res.56.all <- res.56.all %>%
  left_join(mean.vehicle.trip.dur,by=c('age.group'))
...
lt.person.trips <-
  lt.clean %>%
  left_join(chts_per %>% select(SAMPN,PERNO,AGE,EXPPERWGT), by=c('SAMPN','PERNO')) %>%
  filter(ltmode %in% person.trip.modes) %>%
  mutate(age.group=
           ifelse(
             AGE<18,"NA",
...
traveling.persons <-
  ...
  group_by(SAMPN,PERNO) %>%
  summarise(n=n()) %>%
  left_join(lt.person.trips %>% select(SAMPN,PERNO,age.group,EXPPERWGT),
            by=c('SAMPN','PERNO'))

persons.age.group <- traveling.persons %>% 
  group_by(age.group) %>%
  summarise(persons=sum(EXPPERWGT,na.rm=TRUE))
```

Next, we compute the weighted person-trip sums for each county/day.

```{r rt-t56-11}
person.trips.age.group <- 
  lt.age.group.trips %>% group_by(age.group) %>%
  summarise(trips=sum(EXPTCFPERWGT,na.rm=TRUE))
...
person.trips.per.person.age.group <-
  person.trips.age.group %>% 
  left_join(persons.age.group,by=c('age.group')) %>% 
  mutate(ptpp=trips/persons) %>%
  select(-trips,-persons)

res.56.all <- res.56.all %>%
  left_join(person.trips.per.person.age.group,by=c('age.group'))
```

Turning to the person-miles traveled per person, we compute a weighted
sum of the tripdist for each age group and divide by traveling persons.

```{r rt-t56-13}
#PMT per Person
pmt.age.group <-
  lt.age.group.trips %>% group_by(age.group) %>%
  summarise(pmt=sum(EXPTCFPERWGT*tripdist,na.rm=TRUE))

pmt.per.person.age.group <-
  pmt.age.group %>%
  left_join(persons.age.group,by=c('age.group')) %>%
  mutate(pmtpp=pmt/persons) %>%
  select(-pmt,-persons)
...
res.56.all <- res.56.all %>%
  left_join(pmt.per.person.age.group,by=c('age.group'))
```
...
```{r rt-t56-14}
#Average Person Trip Length
avg.person.trip.length.age.group <-
  lt.age.group.trips %>% 
  group_by(age.group) %>%
  summarise(
    avg.person.trip.length=wtd.mean(tripdist,
                                    weights=EXPTCFPERWGT,
                                    na.rm=TRUE))

res.56 <- res.56.all %>%
  left_join(avg.person.trip.length.age.group,by=c('age.group'))
```
...
res.56.all <- res.56 %>%
  filter(age.group!="NA") %>%  # remove youngest
  transmute('Age Group'=age.group,
            'Vehicle Trips per Driver'=`Vehicle Trips per Driver`,
            'Pct Works Trip'=`Pct Work Trips`,
            'Pct Non-Work Trips'=`Pct Non-Work Trips`,
            'VMT per Driver'=vmt.per.driver,
...
# transpose the data to match the requested format
names(res.56.trans) <- as.vector(t(res.56.all['Age Group']))

# reorder cols
res.56.trans <- res.56.trans[,c(4,3,1,2)]

# remote the "age group" row (it's redundant)
res.56.trans <- res.56.trans[-1,]

res.56.str <-
  pandoc.table.return(
    res.56.trans,
    caption="TABLE 56: Daily travel stats by age",
    split.table=Inf,
    split.cells=c(30,rep(10,4)),
...
```{r rt-t-56-results,results='asis',echo=FALSE,cache=FALSE}
cat(res.56.str)
res[["res.56"]] <- list()
res[["res.56"]]$title  <- "TABLE 56: Daily travel stats by age"
res[["res.56"]]$data <- res.56.all
res[["res.56"]]$tabtxt <- res.56.str
```


# Writing Tables for Use with Other Programs

We have saved the data for all tables in the `res` list.  Now we will
write them all out to CSV files so they may be imported into other
programs as needed.

```{r rt-write-csv,cache=FALSE,warn=FALSE}
# Set CSV output dir
csvout <- po("csv")
for (tname in names(res)) {
  fname <- paste(csvout,paste(tname,"csv",sep="."),sep="/")
  # see if rows are named
  rn <- sum(as.integer(rownames(res[[tname]]$data)),na.rm=TRUE)==0 
  write(paste("Writing",fname,rn,"\n"),stderr())

  # Table title
  write.table(res[[tname]]$title,file=fname,row.names=FALSE,col.names=FALSE)            

  # blank row
  write.table("",file=fname,append=TRUE,row.names=FALSE,col.names=FALSE)

  # data
  suppressMessages(
      write.table(res[[tname]]$data,file=fname,append=TRUE,
                  sep=",",col.names=TRUE,row.names=rn))  
}
```
