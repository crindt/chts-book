...
panderOptions(keep.trailing.zeros=TRUE,big.mark=',',round=1)
...
```
```{r t9-2-results,results='asis',echo=FALSE}
cat(res[['9.2.str']])
```
...
aggp <- data2 %>% group_by(County,HHUT,VOWN,TripType) %>% summarise(n=sum(n,na.rm=TRUE))
...
```{r t10a-results,results='asis',echo=FALSE}
cat(res[['10a.str']])
```
...
for (var in sort(unique(res.11$County))) {
  dat<-res.11 %>% filter(County==var) %>%
    ...
    select(
      #County,
      `Housing Unit Type`=HHUT,`Vehicle Ownership`=VOWN,HH,HO,HS,HW,OO,WO,WW,Total=zTOT)
...
```{r t11-results,results='asis',echo=FALSE}
cat(res[['11.str']])
```
...
datapt <- chts_lt %>% filter(!is.na(ltmode) & !is.na(oplano)) %>%
  ...
  left_join(chts_per.wgt.wd,by=c('SAMPN','PERNO')) %>% # join weekday weights (and WD)
  filter(WD==1) %>% # only weekdays
  group_by(SAMPN,PERNO) %>% summarise(npt=sum(NEXPPERWDWGT*ltTCF))
...
datadt <- chts_lt %>% filter(!is.na(ltmode) & !is.na(oplano)) %>%
  ...
  left_join(chts_per.wgt.wd,by=c('SAMPN','PERNO')) %>% # join weekday weights (and WD)
  filter(WD==1) %>% # only weekdays
  group_by(SAMPN,PERNO) %>% summarise(ndt=sum(NEXPPERWDWGT*ltTCF))
...
aggp <- data2 %>% group_by(County,HHUT,HHSIZ) %>% summarise(npt=sum(npt),ndt=sum(ndt,na.rm=TRUE))
aggp.cty.hhut <- data2 %>% 
  mutate(HHSIZ='Total') %>% 
  group_by(County,HHUT,HHSIZ) %>%
  summarise(npt=sum(npt,na.rm=TRUE),ndt=sum(ndt,na.rm=TRUE))
aggp.cty.hhsiz <- data2 %>% 
  mutate(HHUT='Total') %>% 
  group_by(County,HHUT,HHSIZ) %>%
  summarise(npt=sum(npt,na.rm=TRUE),ndt=sum(ndt,na.rm=TRUE))
aggp.cty <- data2 %>% mutate(HHUT='Total',HHSIZ='Total') %>% group_by(County,HHUT,HHSIZ) %>%
  summarise(npt=sum(npt,na.rm=TRUE),ndt=sum(ndt,na.rm=TRUE))
aggp <- rbind(aggp,aggp.cty.hhut,aggp.cty.hhsiz,aggp.cty) %>% ungroup()

# Now, we'll count the total households in each stratifications
datahh.all <- chts_hh %>% mutate(ICTFIP=as.integer(CTFIP)%%1000) %>%
  left_join(fipsctyca,by=c('ICTFIP'='county_ansi')) %>% 
  select(SAMPN,RESTY,HHSIZ,County) %>% 
  mutate(HHUT=ifelse(RESTY==1 | RESTY==2,'Single','Multiple'),
         HHSIZ=ifelse(HHSIZ<5,as.character(HHSIZ),'5+')) %>%
  left_join(chts_hh.wgt.wd,by=c('SAMPN'))  %>% filter(WD==1) %>%
  group_by(County,HHUT,HHSIZ) %>% summarise(hhn=sum(NEXPHHWDWGT))
...
datahh.cty.hhsiz <- chts_hh %>% mutate(ICTFIP=as.integer(CTFIP)%%1000) %>%
  left_join(fipsctyca,by=c('ICTFIP'='county_ansi')) %>% 
  select(SAMPN,RESTY,HHSIZ,County) %>% 
  mutate(HHUT='Total',
         HHSIZ=ifelse(HHSIZ<5,as.character(HHSIZ),'5+')) %>%
...
datahh <- rbind(datahh.all,datahh.cty.hhut,datahh.cty.hhsiz,datahh.cty)

```

```{r table-13a-2}
# take the trip aggs (appg), join the household aggs by stratification, devide totals
agg1 <- aggp %>% left_join(datahh,by=c('County','HHUT','HHSIZ')) %>% 
  mutate(nnpt=npt/hhn,nndt=ndt/hhn) %>% select(-npt,-ndt,-hhn) %>% ungroup()
...
res[['13.str']] = ''
for (var in sort(unique(res.13$County))) {
  dat<-res.13 %>% filter(County==var) %>%
    select(`Household Size`=HHSIZ,
           `PT/HH Single`=Single.nnpt,`PT/HH Multiple`=Multiple.nnpt,`PT/HH Total`=Total.nnpt,
           `DT/HH Single`=Single.nndt,`DT/HH Multiple`=Multiple.nndt,`DT/HH Total`=Total.nndt
           )

  tab <- pandoc.table.return(
    dat,
    caption=paste("Weekday Person and Driver Trips Per Household (",var,")",sep=""),
    split.table=Inf,split.cells=Inf,
    big.mark=',',
    round=1,
    #               justify=c('left','left','right','right','right'),
    style='multiline')
  res[['13.str']] <- paste(res[['13.str']],tab,sep="\n\n")
}
```
...
aggp.all <- data2 %>% group_by(County,HHUT,INCOM) %>% summarise(n=sum(n))
aggp.cty.hhut <- data2 %>% 
  mutate(INCOM='Total') %>%
  group_by(County,HHUT,INCOM) %>%
  summarise(n=sum(n))
aggp.cty.incom <- data2 %>%
  mutate(HHUT='Total') %>%
  group_by(County,HHUT,INCOM) %>%
  summarise(n=sum(n))
aggp.cty <- data2 %>%
...
aggp <- rbind(aggp.all,aggp.cty.hhut,aggp.cty.incom,aggp.cty)
...
datahh.all <- hhtt %>% 
  mutate(HHUT=ifelse(RESTY==1 | RESTY==2,'Single','Multiple')) %>%
  left_join(chts_hh.wgt.wd,by=c('SAMPN'))  %>% filter(WD==1) %>%
  group_by(County,HHUT,INCOM) %>% summarise(hhn=sum(NEXPHHWDWGT*HHVEH))
datahh.cty.hhut <- hhtt %>% 
  mutate(HHUT=ifelse(RESTY==1 | RESTY==2,'Single','Multiple'),
         INCOM='Total'
         ) %>%
  left_join(chts_hh.wgt.wd,by=c('SAMPN'))  %>% filter(WD==1) %>%
  group_by(County,HHUT,INCOM) %>% summarise(hhn=sum(NEXPHHWDWGT*HHVEH))
datahh.cty.incom <- hhtt %>% 
  mutate(HHUT='Total',
         INCOM=INCOM
         ) %>%
  left_join(chts_hh.wgt.wd,by=c('SAMPN'))  %>% filter(WD==1) %>%
...
datahh <- rbind(datahh.all,datahh.cty.hhut,datahh.cty.incom,datahh.cty)
```
...
library(tidyr)
res.15 <- agg1  %>%
  arrange(County,HHUT,INCOM) %>% 
  spread(HHUT,nn) %>% # reorganize for table
  select(County,INCOM,Single,Multiple,Total)

res[['15.str']] = ''
for (var in sort(unique(res.15$County))) {
  dat<-res.15 %>% filter(County==var) %>%
    select(`Household Income`=INCOM,Single,Multiple,Total)

  tab <- pandoc.table.return(
    dat,
    caption=paste("Weekday Driver Trips Per Vehicle (",var,")",sep=""),
    split.table=Inf,split.cells=Inf,
    big.mark=',',
    round=1,
    #               justify=c('left','left','right','right','right'),
    style='multiline')
  res[['15.str']] <- paste(res[['15.str']],tab,sep="\n\n")
}
```
...
for (var in sort(unique(res.15a$County))) {
  ...
  tab <- pandoc.table.return(
    dat,
    caption=paste("Person and Vehicle Miles Traveled per Household (",var,")",sep=""),
    split.table=Inf,split.cells=Inf,
    big.mark=',',
    round=1,
    #               justify=c('left','left','right','right','right'),
...
```{r t17a-results,echo=FALSE,results='asis'}
cat(res[['17a.cty.str']])
```
