...
We have saved the data for all tables in the `res` list.  Now we will write them all out to 
CSV files so they may be imported into other programs as needed.
...
for (tname in names(res)) {
  ...
  rn <- sum(as.integer(rownames(res[[tname]]$data)),na.rm=TRUE)==0 # see if rows are named
