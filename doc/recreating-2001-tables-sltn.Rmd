...
res10a <- rbind(agg1,agg2)  %>%
  arrange(County,HHUT,VOWN,TripType)
pandoc.table(res10a,
               caption=paste("Weekday Person Trips Per Household",sep=""),
               split.table=Inf,split.cells=Inf,
...
Now, `data` has the linked trips for each weekday person trip.  Once again, we can simply compute
the mean for each combination.

```{r table-11-2,results='asis'}
# take the trip aggs (appg), join the household aggs by stratification, devide totals
agg1 <- aggp %>% left_join(datahh,by=c('County','HHUT','VOWN')) %>% mutate(nn=n/hhn) %>% select(-n,-hhn) %>% ungroup()
# Sum the totals for each trip type
agg2<-agg1 %>% group_by(County,HHUT,VOWN) %>% summarise(nn=sum(nn)) %>%
  mutate(TripType="zTOT")
res11<-rbind(agg1,agg2)  %>%
  arrange(County,HHUT,VOWN,TripType)
pandoc.table(res11,
               caption=paste("Weekday Driver Trips Per Household",sep=""),
               split.table=Inf,split.cells=Inf,
               justify=c('left','left','right','right','right'),
               style='multiline')
```
