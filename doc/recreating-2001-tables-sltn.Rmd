...
```{r rt-setup-0,echo=FALSE,cache=TRUE}
...
```{r rt-recreating-2001-tables-setup,cache=TRUE,echo=TRUE}
...
```{r rt-write-csv,cache=TRUE,warn=FALSE}
# Set CSV output dir
csvout <- po("csv")
...
```{r rt-dump-all,echo=FALSE,results='asis',cache=TRUE}
cat(
  paste(
    lapply(
      names(res),
