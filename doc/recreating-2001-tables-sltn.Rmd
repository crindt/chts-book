...
res[['b.0.str']] <- pandoc.table.return(
  res.b.0,
  style='multiline',
  split.tables=Inf,
  split.cells=Inf,
...
lic.reg <-  chts_hh %>% 
  ...
  group_by(County) %>%
  summarise(
    Licensed=round(wtd.mean(HHLIC,weights=NEXPHHWGT),1)
    )

res.5 <- rbind(lic.ca,lic.reg)
...
emp.reg <- chts_hh %>% 
  ...
  group_by(County) %>%
  summarise(
    Employed=round(wtd.mean(HHEMP,weights=NEXPHHWGT),1)
    )
...
res[['9.str']] <- 
  pandoc.table.return(
    res.9.1,caption="Weekday Trip Type Distribution",
...
```{r trip-dist-by-county,results='asis'}
data<-ltf2 %>% 
  filter(!is.na(dplano)) %>%
...
res[['9.2.str']] <- ''
for (var in sort(unique(data$County))) {
  dat<-data %>% filter(County==var)
  ...
  tabtot.str <- pandoc.table.return(tabtot,
  ...
  cat(tabtot.str)
  res[['9.2.str']] <- paste(res[['9.2.str']],tabtot.str,sep="\n")
}
```
...
data <- chts_lt %>% filter(!is.na(ltmode) & !is.na(oplano)) %>%
  ...
  group_by(SAMPN,PERNO,TripType) %>% summarise(n=sum(NEXPPERWDWGT*ltTCF))
...
aggp <- data2 %>% group_by(County,HHUT,VOWN,TripType) %>% summarise(n=sum(n))
aggp.cty.hhut <- data2 %>% mutate(VOWN='Subtotal') %>% group_by(County,HHUT,VOWN,TripType) %>%
  summarise(n=sum(n))
aggp.cty <- data2 %>% mutate(HHUT='Total',VOWN='&nbsp;') %>% group_by(County,HHUT,VOWN,TripType ) %>%
  summarise(n=sum(n))
aggp <- rbind(aggp,aggp.cty.hhut,aggp.cty)

# Now, we'll count the total households in each stratifications
datahh <- chts_hh %>% mutate(ICTFIP=as.integer(CTFIP)%%1000) %>%
  left_join(fipsctyca,by=c('ICTFIP'='county_ansi')) %>% 
  select(SAMPN,RESTY,HHVEH,County) %>% 
  mutate(HHUT=ifelse(RESTY==1 | RESTY==2,'Single','Multiple'),
         VOWN=ifelse(HHVEH<3,as.character(HHVEH),'3+')) %>%
  left_join(chts_hh.wgt.wd,by=c('SAMPN'))  %>% filter(WD==1) %>%
  group_by(County,HHUT,VOWN) %>% summarise(hhn=sum(NEXPHHWDWGT))

datahh.cty.hhut <- chts_hh %>% mutate(ICTFIP=as.integer(CTFIP)%%1000) %>%
  left_join(fipsctyca,by=c('ICTFIP'='county_ansi')) %>% 
  select(SAMPN,RESTY,HHVEH,County) %>% 
  mutate(HHUT=ifelse(RESTY==1 | RESTY==2,'Single','Multiple'),
         VOWN='Subtotal') %>%
...
datahh.cty <- chts_hh %>% mutate(ICTFIP=as.integer(CTFIP)%%1000) %>%
  left_join(fipsctyca,by=c('ICTFIP'='county_ansi')) %>% 
  select(SAMPN,RESTY,HHVEH,County) %>% 
  mutate(HHUT='Total',
         VOWN='&nbsp;') %>%
...
res[['10a.str']] = ''
for (var in sort(unique(res.10a$County))) {
  dat<-res.10a %>% filter(County==var) %>%
    mutate(County=ifelse(!is.na(lag(County)) & County==lag(County),"&nbsp;",County),
           HHUT=ifelse(!is.na(lag(HHUT)) & HHUT==lag(HHUT),"&nbsp;",HHUT)
    ) %>%
    select(County,`Housing Unit Type`=HHUT,`Vehicle Ownership`=VOWN,HH,HO,HS,HW,OO,WO,WW,Total=zTOT)


  tab <- pandoc.table.return(
    dat,
    caption=paste("Weekday Person Trips Per Household (",var,")",sep=""),
    split.table=Inf,split.cells=Inf,
    big.mark=',',
    round=1,
    ...
    style='multiline')
  cat(tab)
  res[['10a.str']] <- paste(res[['10a.str']],tab,sep="\n\n")
}
```
...
res[['11.str']] = ''
for (var in sort(unique(res.11$County))) {
  dat<-res.11 %>% filter(County==var) %>%
    mutate(County=ifelse(!is.na(lag(County)) & County==lag(County),"&nbsp;",County),
           HHUT=ifelse(!is.na(lag(HHUT)) & HHUT==lag(HHUT),"&nbsp;",HHUT)
    ) %>%
    select(County,`Housing Unit Type`=HHUT,`Vehicle Ownership`=VOWN,HH,HO,HS,HW,OO,WO,WW,Total=zTOT)
  ...
  tab <- pandoc.table.return(
    dat,
    caption=paste("Weekday Driver Trips Per Household (",var,")",sep=""),
    split.table=Inf,split.cells=Inf,
    big.mark=',',
    round=1,
    #               justify=c('left','left','right','right','right'),
    style='multiline')
  cat(tab)
  res[['11.str']] <- paste(res[['11.str']],tab,sep="\n\n")
}
```
...
```{r table13a}
# By Region, Household Size, Housing Unit Type
```
```{r}
# compute person trips per category
# Filter for weekday person trips and count the expanded total trips per type
data <- chts_lt %>% filter(!is.na(ltmode) & !is.na(oplano)) %>%
  ...
  left_join(chts_per.wgt.wd,by=c('SAMPN','PERNO')) %>% # join weekday weights (and WD)
  filter(WD==1) %>% # only weekdays
  group_by(SAMPN,PERNO) %>% summarise(n=sum(NEXPPERWDWGT*ltTCF))
...
data2 <- data  %>% 
  left_join(chts_hh %>% mutate(ICTFIP=as.integer(CTFIP)%%1000) %>%
              ...
              mutate(HHUT=ifelse(RESTY==1 | RESTY==2,'Single','Multiple'),
                     HHSIZx=ifelse(HHSIZ<5,as.character(HHSIZ),'5+')), by=c('SAMPN'))

# Now, aggregate the expanded driver trip totals by the stratifications
aggp <- data2 %>% group_by(County,HHUT,HHSIZx) %>% summarise(n=sum(n))
...
datahh <- chts_hh %>% mutate(ICTFIP=as.integer(CTFIP)%%1000) %>%
  ...
  select(SAMPN,RESTY,HHSIZ,County) %>% 
  mutate(HHUT=ifelse(RESTY==1 | RESTY==2,'Single','Multiple'),
         HHSIZx=ifelse(HHSIZ<5,as.character(HHSIZ),'5+')) %>%
  left_join(chts_hh.wgt.wd,by=c('SAMPN'))  %>% filter(WD==1) %>%
  group_by(County,HHUT,HHSIZx) %>% summarise(hhn=sum(NEXPHHWDWGT))
```

We also want the totals, without the household type stratification

```{r table-13a-3}
aggt <- data2 %>% group_by(County,HHSIZx) %>% summarise(n=sum(n))

datahht <- chts_hh %>% mutate(ICTFIP=as.integer(CTFIP)%%1000) %>%
  left_join(fipsctyca,by=c('ICTFIP'='county_ansi')) %>% 
  select(SAMPN,HHSIZ,County) %>% 
  mutate(HHSIZx=ifelse(HHSIZ<5,as.character(HHSIZ),'5+')) %>%
  left_join(chts_hh.wgt.wd,by=c('SAMPN'))  %>% filter(WD==1) %>%
  group_by(County,HHSIZx) %>% summarise(hhn=sum(NEXPHHWDWGT))

```


Now, `data` has the linked trips for each weekday person trip.  We can simply compute the mean for
each combination.

```{r table-13-2}
# take the trip aggs (appg), join the household aggs by stratification, devide totals
agg1 <- aggp %>% left_join(datahh,by=c('County','HHUT','HHSIZx')) %>% mutate(nn=n/hhn) %>% select(-n,-hhn) %>% ungroup()
# Sum the totals for each trip type
agg2 <- aggt %>% left_join(datahht,by=c("County","HHSIZx")) %>% group_by(County,HHSIZx) %>% summarise(nn=n/hhn) %>%
  mutate(HHUT='Total') %>% ungroup()
res.13a<-rbind(agg1,agg2)  %>%
  arrange(County,HHSIZx,HHUT) %>%
  select(County,HHSIZx,HHUT,nn)    # reorder columns
```


```{r table13a-output,results='asis'}
res[['13a.str']] <- pandoc.table.return(res.13a)
```
...
# Now, aggregate the expanded driver trip totals by the stratifications
aggp <- data2 %>% group_by(County,HHUT,INCOM) %>% summarise(n=sum(n))
...
datahh <- chts_hh %>% mutate(ICTFIP=as.integer(CTFIP)%%1000) %>%
  ...
  mutate(HHUT=ifelse(RESTY==1 | RESTY==2,'Single','Multiple')) %>%
  left_join(chts_hh.wgt.wd,by=c('SAMPN'))  %>% filter(WD==1) %>%
  group_by(County,HHUT,INCOM) %>% summarise(hhn=sum(NEXPHHWDWGT))
```
...
```{r table-15-2}
aggt <- data2 %>% group_by(County,INCOM) %>% summarise(n=sum(n))
...
```{r table-15-3}
# take the trip aggs (appg), join the household aggs by stratification, devide totals
agg1 <- aggp %>% left_join(datahh,by=c('County','HHUT','INCOM')) %>% mutate(nn=n/hhn) %>% select(-n,-hhn) %>% ungroup()
# Sum the totals for each trip type
agg2 <- aggt %>% left_join(datahht,by=c("County","INCOM")) %>% group_by(County,INCOM) %>% summarise(nn=n/hhn) %>%
  mutate(HHUT='Total') %>% ungroup()
res.15<-rbind(agg1,agg2)  %>%
  arrange(County,INCOM,HHUT) %>%
  select(County,INCOM,HHUT,nn) %>%   # reorder columns
  select(County,INCOM,`Household Type`=HHUT,`Driver Trips`=nn)

res.15$INCOM <- assign_factors(res.15$INCOM,'hh','INCOM')
```


```{r table15-output,results='asis'}
res[['15.str']]<-pandoc.table.return(res.15)
cat(res[['15.str']])
```



## TABLE 15a: WEEKDAY PERSON AND VEHICLE MILES TRAVELED PER VEHICLE BY COUNTY, HH INCOME, AND HOUSING UNIT
...
# Now, aggregate the expanded driver trip totals by the stratifications
aggp <- data2 %>% group_by(County,HHUT,INCOM) %>% summarise(n=sum(n),nv=sum(nv))
...
```{r table-15a-2}
aggt <- data2 %>% group_by(County,INCOM) %>% summarise(n=sum(n),nv=sum(nv))
...
```{r table-15a-3}
# take the trip aggs (appg), join the household aggs by stratification, devide totals
agg1 <- aggp %>% left_join(datahh,by=c('County','HHUT','INCOM')) %>% mutate(nn=n/hhn,nnv=nv/hhn) %>% select(-n,-nv,-hhn) %>% ungroup()
# Sum the totals for each trip type
agg2 <- aggt %>% left_join(datahht,by=c("County","INCOM")) %>% group_by(County,INCOM) %>% summarise(nn=n/hhn,nnv=nv/hhn) %>%
  mutate(HHUT='Total') %>% ungroup()
res.15a<-rbind(agg1,agg2)  %>%
  arrange(County,INCOM,HHUT) %>%
  select(County,INCOM,HHUT,nn,nnv) %>%   # reorder columns 
  select(County,Income=INCOM,`Household Type`=HHUT,PMT=nn, VMT=nnv)
res.15a$Income <- assign_factors(res.15a$Income,'hh','INCOM')
```


```{r table15a-output,results='asis'}
res[['15a.str']]<-pandoc.table.return(res.15a)
cat(res[['15a.str']])
```
...
```{r dump-all,echo=FALSE,results='asis'}
