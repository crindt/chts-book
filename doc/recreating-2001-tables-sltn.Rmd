...
Weights used in the calculations of the following tables were generated separately

Here we load in various weights used in the calculations below, including weekday weights and 
county weights (and county weekday weights).
...
hhvehinuselabel <- paste('Vehicles In Use on Average Weekday (',
            ...
            '%)',sep="")
adat[[hhvehinuselabel]]  <- round(sum(hhvehinuse$NEXPHHWDWGT),0)
```
...
bdat[["&nbsp;&nbsp;Weekday 'All Trips' Trip Duration (min)"]] <- 
  round(
...
res.c <- res.c %>% arrange(desc(ifelse(Weekday.Percent>99.9999,-Weekday.Percent,Weekday.Percent)))
  ...
  paste(
    lapply(
      ...
      function(icty) {
        ...
        tab.ul <- 
          tab.ul.raw %>%
          filter(grepl("\\.1$",GROUP) & (CTFIP==icty | oCTFIP==icty)) %>%
        ...
        tab <- 
          tab.raw %>%
          filter(oCTFIP==icty | dCTFIP==icty) %>%
          summarise(num=sum(NEXPTCFPERWDCTYWGT,na.rm=TRUE))
        #  summarise(num=n())
        tab$ltmode <- assign_factors(tab$ltmode,'place','MODE',maxlab = 20)
        tots <- tab %>% ungroup() %>% group_by(MODE.GROUP) %>% summarise(ltmode="TOTAL",tnum=sum(num))
...
cat(res.53.str)
res[["res.53"]] <- list()
res[["res.53"]]$title  <- "TABLE 52: Statewide Commute trips (HW) by Mode (Walk, Bike, Drive/Passenger, Transit) and other by Mode and Distance ranges"
