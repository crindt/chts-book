...
This document explains how county-level weights are developed for long distance trips using the R statistical software for people who participated in the 2012 CHTS. The purpose of these weights is to scale up information provided by respondents to the population of each county based on the socio-economic variables listed below. The person level weights are generated for long distance trips at  county level. Long distance travel file did not properly identify the person identification number. So we have to rely on variable LNINI1 (person who made the trip) and there are 1310 missing observations. Users of these weights are cautioned that these weights should not be applied to lower-level geographies such as cities or zip codes. Similar weights can be developed for these geographies if they contain enough CHTS respondents.

## Methodology

County wide Weights for Long distance trips  are calculated by raking at the person level. These weights adjust the relative importance of responses to reflect the different probabilities of selecting respondents, and align the sample distributions to population distributions based on the 2010 Census. In particular, long distance person-level weights were adjusted so that the sum of these weights equals known population totals for certain subgroups of the population of each county. Variables used for raking at the person level are:
...
After the raking procedure, very large weights are capped to be no more than five times the mean of weight. 

## R Code and Implementation

The following commands are used to generate county wide weights for long distance trips.


### Generate weight for long distance trip
...
setwd("~/Suman_research/Caltrans Project/Weight/PersonWeight_County")
...
tot.pop <- read.csv("c:/Users/user/Documents/Suman_research/Caltrans Project/Weight/Data_County/Total/county_totpop2.csv", header=TRUE)
head(tot.pop)
...
So, now we have a new file named perdata3 which contains all the raking variables informaiton and county wide total population. 

Now, We need to import the Long distance file  and corresponding look up file (containing location information) and merge them.

```{r,cache=TRUE}
setwd("~/Suman_research/Caltrans Project/Weight/LD_Weight2")
...
Here, we delete the duplicate variables and give new names of the added variables.
...
There is no person id information in long distance file. But we need person id to generate person level weight. So we generate a person id variable from the variable person who made trip (LDINI1).
...
In this data set there are observations for persons who did not take the long distance trip. We exclude those observations. 
...
This new merged file contains all county level information. But we need a single dataframe for each county. So we need to create a list of seperate dataframe for each county.
...
```{r,cache=TRUE}
# Hispanic data
hisp.county <- read.csv("c:/Users/user/Documents/Suman_research/Caltrans Project/Weight/Data_County/Hispanic/Hisp_county.csv", header=TRUE)
head(hisp.county)
...
```{r,cache=TRUE}
# Race
race.county <- read.csv("c:/Users/user/Documents/Suman_research/Caltrans Project/Weight/Data_County/Race/Race_county.csv", header=TRUE)
...
```{r,cache=TRUE}
## Employment data
emply.county <- read.csv("c:/Users/user/Documents/Suman_research/Caltrans Project/Weight/Data_County/Employment/Employment_county.csv", header=TRUE)
...
```{r,cache=TRUE}
#Gender
gend.county <- read.csv("c:/Users/user/Documents/Suman_research/Caltrans Project/Weight/Data_County/Gender/Gender_county.csv", header=TRUE)
...
# Age
age.county <- read.csv("c:/Users/user/Documents/Suman_research/Caltrans Project/Weight/Data_County/Total/age_county.csv", header=TRUE)
head(age.county)
...
As our data is ready for raking, we proceed for raking next. We  do it in a for loop function which generate weight for each county. To do this we need to install the library "survey". First we design a survey using 'svydesign' command and we use sampling without replacement (pps ="brewer") option. Since sampling information is not available for long distance data, we assume equal probability here. So we don't need to specify the fpc option here. 
...
for (i in 1:length(hisp.data)){ #length must be equal to 58=total number of county
  tryCatch({ #If there is any error it will give error message but still continue the loop 
    ...
    s<-as.character(f[i])
    name=c('LDweight_',s, '.CSV')
    name=paste(name,collapse="")
    temp1<- svydesign(id=~1,data=ldcnty_list[[i]],pps="brewer") # Survey design with equal probability
    ...
    colnames(temp5)[4] <- "EXPLDWGT" # change the column name of weight variable
...
for (i in d){ 
  tryCatch({
    ...
    name=paste(name,collapse="")
    temp1<- svydesign(id=~1,data=ldcnty_list[[i]], pps="brewer") # Survey design with equal probability
    temp3<- rake(temp1, list(~HISPn,~EMPLYn,~GENDn), list(hisp.data[[i]],emply.data[[i]],gend.data[[i]]),control = list(maxit = 100)) # Raking
    temp4<-trimWeights(temp3,lower=(min(weights(temp3))),upper=(5*mean(weights(temp3)))) # Trim the weights (max value no greater than 5 times the mean)
    temp5<-cbind(temp4$variables[1:3],weights(temp4)) # Combine weight with sample no and person no
    colnames(temp5)[4] <- "EXPLDWGT" # change the column name of weight variable
    write.table(temp5, file =name,sep=",", row.names=FALSE, col.names=TRUE) # Save as csv file
...
There is no error now. So, we have expanded person weights for long distance trips for all 58 counties and these weights are saved in the directory we specified earlier. We need to combine these weights of 58 counties into one file. The following codes are used to import and combine them.


```{r,cache=TRUE}
# Import multiple csv file of new expanded weight and combine into one file
setwd("~/Suman_research/Caltrans Project/Weight/LD_Weight")
filenames <- list.files(path = "~/Suman_research/Caltrans Project/Weight/LD_Weight") # import the files
nldweight2<-do.call("rbind", lapply(filenames, read.csv, header = TRUE)) # combined the files into one
head(nldweight2)
dim(nldweight2)
```
...
```{r,cache=TRUE}
setwd("~/Suman_research/Caltrans Project/Weight/LD_Weight2")
write.csv(nldweight2, file="nldweight922015.csv",row.names = FALSE) # save as csv
```
...
Now we have a long distance data file with new county wide weight and we want to save it as csv.

```{r,cache=TRUE}
write.csv(wLDdata, file="wLD_data922015.csv",row.names=FALSE)
```
