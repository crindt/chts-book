...
```{r ldw-setup-0,echo=FALSE,cache=FALSE}
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Generate weight for long distance trip')
```
...
for (i in 1:length(hisp.data)){ #length must be equal to 58=total number of county
  tryCatch({ #If there is any error it will give error message but still continue the loop 
    ...
    name=p(paste("Weight/LD_Weight",name,sep="/"))
...
setwd(p("Weight/LD_Weight"))
filenames <- list.files(path = p("Weight/LD_Weight")) # import the files
...
```{r ldw-25,cache=TRUE}
write.csv(wLDdata, file=p(paste("Weight/LD_Weight2","wLD_data922015.csv",sep="/")),row.names=FALSE)
```
