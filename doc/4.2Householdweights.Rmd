...
the R statistical software for household who participated in the 2012
...
socio-economic variables listed below. The household level weights
were developed at the county level. Users of these weights are
...
developed for these geographies if they contain enough CHTS
respondents.
...
responses to reflect the different probabilities of selection of
...
to known population totals for certain subgroups of the population of
each county.  Following NuStats' work for Caltrans, variables used for
...
the new weights align with the population of the different counties.
...
data frame (containing location information) and then we merge them.
...
Now we have new merged file which is used to generate new household
...
Then, we change the value of household income (`INCOM`) variable to make
it similar to census data.
...
Here, we recode number of household vehicles (`HHVEH`) variable.
...
Now we have data frame (`nhhdata`) which has similar coding as census
data for raking variables.  We need county wide total household data
which we collect from census and save it as CSV (we organize them as
per our requirement).  Here we import that file and merge this data
with `nhhdata`.
...
So, now we have a new data frame named `hhdata3` which contains all the
raking variables information and county wide total household. This is
a single file for all counties.  But we need separate dataframes for
each county. So we need to create a list of separate dataframes for
each county containing all the information.
...
We need census data of raking variables (Household size, Household
income, Number of household vehicles and Number of household
workers). We collect this information from census and organize them as
per our requirement and save as CSV file. Then we import them and
create separate dataframes for each county (the csv file contain
information of all the counties).

First, we do it for Household size variable.

```{r hw-11,cache=TRUE}
# Household size data
hhsize.county <- read.csv(p("Weight/Data_County/County_HH/HHSize/hhsize.csv"),
                          header=TRUE)
head(hhsize.county)
...
```{r hw-12,cache=TRUE}
# Income
hhincome.county <- read.csv(p("Weight/Data_County/County_HH/Income/hhincome.csv"),
                            header=TRUE)
...
hhveh.county <- read.csv(p("Weight/Data_County/County_HH/HHVeh/hhvehicle.csv"),
                         header=TRUE)
head(hhveh.county)
...
Finally, we import the number of household number of workers.

```{r hw-14,cache=TRUE}
## Number of workers
hhwrk.county <- read.csv(p("Weight/Data_County/County_HH/Worker/hhworkers.csv"),
                         header=TRUE)
head(hhwrk.county)
...
using the `svydesign` command and we use sampling without replacement
(pps ="brewer") option. The option 'fpc (finite population
correction)' is the ratio of sample household number and county wide
total household. Then we perform the raking. After raking, we trim the
weight so that very large weights were capped to be no more than five
times the mean of weight.The following codes also give us information
about the errors (the FIP code of counties for which the weight can't
be generated). The generated expanded household weights for different
...
for (i in 1:length(hhwrk.data)){ 
  tryCatch({
    ...
    name=p(paste("Weight/HHWeight_County",name,sep="/"))
...
After running the codes, we get two errors for two county data sets
(`HCTFIP={3,43}`) and we need to treat them individually. The errors show
that some `HHVEHC` (number of household vehicles) categories are absent
from sample of these counties, so we don't include `HHVEHC` as raking
variable for these counties in the following codes.
...
There is no error now. So, we have expanded household weights for all
58 counties and these weights are saved in the directory we specified
earlier. We need to combine these weights of 58 counties into one
file. The following code imports and combines them.
...
Now we have on file which contains new county wide expanded household
weight for all counties. We want to generate new county wide household
weight (not expanded) from our expanded weight. We know that `household
...
Now we have a file with new county wide expanded and unexpanded
household weights. Here we save it in our desired directory.

```{r hw-22,cache=TRUE}
# Save 
setwd(p("Weight/NewWeightData"))
write.csv(nhhweight2, file="nhhweight922015.csv",
          row.names = FALSE) 
```

We want to merge the new weight with household data file and save it
as CSV.
...
Now we have a household data file with new county wide household
weight.  As we mentioned earlier we want check whether the new weights
represent the total household of the counties. So we perform some
random checks for different variables using following codes. We do it
for Alameda county as an example.
...
The total household of Alameda county is 53,2026 (based on
census). Our random check shows that the new weights align with the
total household of Alameda county for both variables.
