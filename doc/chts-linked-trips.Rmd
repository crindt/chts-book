...
```{r lt-setup,cache=TRUE,echo=FALSE}
...
tab <-
    data.frame(
        ...
        `CHTS 2011`=c(
            ...
            '`PERNO`',
            'N/A',
            'To be computed from `ACTNO`',
            'To be computed from `ACTNO`',
            'To be computed from `PLANO`',
            'To be computed from `PLANO`',
            'N/A',
            'N/A',
            'Unknown values in `{1,2}`',
            'Interpreted as mode of longest distance trip in set of linked trips',
            'Definition unclear.  Values: in `{S,W,L,D,I,B,P,T,A,O}`',
            'Mapping between conventional 4-step model trip ends: `{HW,HS,OO,WO,HO}',
            'Departure hour: `DEP_HR`',
            'Departure minute: `DEP_MIN`',
            'Arrival hour: `ARR_HR`',
            'Arrival minute: `ARR_MIN`',
            'Total duration of all linked trips combined',
            'Duration of all ignored activities (those with `APURP=c(21,22)`)',
            "Whether a vehicle we available for this trip.  It's unclear if we can compute this",
            'Household vehicle used for trip (if any)',
            'Number of people on the linked trip.  We will assume this refers to the primary trip',
            'Day of month weekday weight?  Unclear whether there is a 2011 analogue',
            'Day of month weekend weight?  Unclear whether there is a 2011 analogue',
            'Day of month ? weight?  Unclear whether there is a 2011 analogue',
            'Day of month ? weight?  Unclear whether there is a 2011 analogue',
            'Day of month ? weight?  Unclear whether there is a 2011 analogue',
            'Original(?) Day of month ? weight?  Unclear whether there is a 2011 analogue',
            'Original(?) Day of month ? weight?  Unclear whether there is a 2011 analogue',
            '?')
    )
pandoc.table(
    tab,
    caption="Mapping between 2001 and 2011 linked trip data",
    split.table=Inf,
    split.cells=Inf,
    style='multiline',
    justify=c('left','left')
...
## Linked trip MODE (Identifying the principal trip)
...
## Linked trip origin and destination
...
# Generating Linked Trip Tables Using R
...
```{r lt-setup-1,cache=TRUE,echo=TRUE}
setwd(rd)
...
```
```{r lt-group-linked-trips-test,cache=TRUE}
...
```{r lt-show-linked-trip-numbering,echo=TRUE,cache=TRUE}
testhhrows <- lttest %>%
  filter(SAMPN==testhh) 
print.data.frame(
        testhhrows %>% 
...
```
```{r lt-expl-calcs,echo=FALSE,cache=TRUE}
per <- 1
pla <- 2
...
## Linked trip MODE (and Trip Correction Factor [TCF])
...
actmap <- read.csv(pd(paste('linked-trips/data',
                            'activity-type-map-2.csv',
                            sep="/")),header=TRUE) %>% 
...
All the others look reasonable except the 8706 `HH` trips and 915 `WW`
trips, which don't make any sense on the surface.  However, if we look
at an example, we can see that these are the result of imperfect
mapping between activity purpose and trip types.  Let's find an
example:
...
## Linked trip departure and arrival times
...
write.csv(chts_lt,file=pd('linked-trips/chts_lt.csv'))  # as csv too, for non-R users
write.csv(chts_lu_lt,file=pd('linked-trips/chts_lu_lt.csv')) # ...
```

The final table contains the following data:

```{r lt-data-defs,echo=FALSE,results='asis',cache=TRUE}
...
```{r lt-lookup-data-defs,echo=FALSE,results='asis',cache=TRUE}
