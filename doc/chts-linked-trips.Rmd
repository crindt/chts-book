...
```{r lt-setup,cache=FALSE,echo=TRUE}
...
```{r read-acts-discussion,echo=TRUE,eval=FALSE}
...
```{r show-linked-trip-numbering,echo=TRUE,cache=FALSE}
testhhrows <- lttest %>%
  filter(SAMPN==testhh) 
print.data.frame(
        testhhrows %>% 
...
```
```{r expl-calcs,echo=FALSE,cache=FALSE}
per <- 1
pla <- 2
...
```{r rank-act-pla-modes,echo=TRUE}
ltr <- lt %>%
...
```{r show-mode-ranks-test-calcs,echo=FALSE,results=FALSE}
...
```{r compute-origin-rank,echo=TRUE}
...
```{r compute-dest-rank1, echo=TRUE}
ltrd1 <- 
...
distinct_types_by_ltno <- lte %>% group_by(SAMPN,PERNO,LTNO,TYPE) %>% summarize()
```
...
```{r four-step-type-summarize}
four_step_types %>% group_by(TripType) %>% summarize(n=n())
```
...
```{r trip-end-troups-show-1,results='asis'}
...
pandoc.table(hbw,caption="Home-based Work",
             ...
             style='multiline')
```
```{r trip-end-troups-show-2,results='asis'}
...
pandoc.table(hbw,caption="Home-based Shopping",
             ...
             style='multiline')
```
```{r trip-end-troups-show-3,results='asis'}
...
pandoc.table(hbo,caption="Home-based Other",
             ...
             style='multiline')
```
```{r trip-end-troups-show-4,results='asis'}
...
pandoc.table(wbo,caption="Work-based Other",
             ...
             style='multiline')
```
```{r trip-end-troups-show-5,results='asis'}
...
record.  `dpylr`'s `summarize` function provides this capability.  Any
...
```{r print-joined-testhh,echo=TRUE}
...
```{r read-lookup-places, echo=TRUE}
...
```{r write-csv-table, echo=TRUE, cache=FALSE}
...
```{r data-defs,echo=FALSE,results='asis',cache=FALSE}
...
```{r lookup-data-defs,echo=FALSE,results='asis',cache=FALSE}
