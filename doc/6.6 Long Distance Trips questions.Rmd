...
Tables for number of non-commute, long distance trips by trip mode for
the counties in the following geographic areas:
...
We have generated weighted and unweighted tables for both origins and
destinations of the above districts.  We have used long distance
recall survey table to generate the results. Note that in the long
distance recall survey, trips with 50 miles or more travel distance
have been defined as long distance trips.We have excluded the work
trip to calculate the non-commute trips. We have used the new county
wide Long distance weights (described in the section on
[Weights for Long Distance Trips] to generate weighted results.
...
To answer the questions we need the long distance table (with new
weights) that we computed in the section on
...
wLDdata <- read.csv(p("Weight/LD_Weight2/wLD_data922015.csv"), header=TRUE)
names(wLDdata)
```
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips from District 1')
```
...
So `Odistrict1` now contains the non-commute long distance trips
originiating in District 1 with appropriate expansion weights
appended.  Now we can generate observations for modal split for
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips from District 2')
```

Next, we perform a similar computation for total number of non-commute
long distance trips from District 2, by mode.

First we subset the long distance data for trips originating in
District 2.
...
Then, we subset the non-commute long distance trips originating in
District 2.
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips from District 3')
```


Here we determine total number of non-commute long distance trips from
District 3, by mode.

First we subset the long distance non-commute trips originating in
District 3.
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips from District 4')
```


We compute total number of non-commute long distance trips from
District 4, by mode.
...
## Creat subset
odist4<-subset(wLDdata,
               LDOCTFIP==1|LDOCTFIP==13|LDOCTFIP==41|LDOCTFIP==55|
                 LDOCTFIP==81|LDOCTFIP==75|LDOCTFIP==85|LDOCTFIP==95|
                 LDOCTFIP==97)
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips from District 5')
```
...
## Creat subset
odist5<-subset(wLDdata,
               LDOCTFIP==53|LDOCTFIP==69|LDOCTFIP==79|
                 LDOCTFIP==83|LDOCTFIP==87)
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips from Southern California')
```


We compute total number of non-commute long distance trips from
Southern California, by mode.
...
## Creat subset
oSC<-subset(wLDdata,
            LDOCTFIP==19|(LDOCTFIP==71 & LDOTRACT>=103 & LDOTRACT<5000)|
              LDOCTFIP==31|LDOCTFIP==39|LDOCTFIP==107|LDOCTFIP==29|
                LDOCTFIP==37|LDOCTFIP==59|LDOCTFIP==73|LDOCTFIP==25|
                LDOCTFIP==111|LDOCTFIP==65)
table(oSC$LDOCTFIP)
# Subset for non-commute trips
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips from Southern District 9')
```
...
## Creat subset
odist9<-subset(wLDdata,LDOCTFIP==27|LDOCTFIP==51|
                         (LDOCTFIP==71 & LDOTRACT>=5000))
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips to District 1')
```


Now we perform a similar computation for trips *to* District 1.  As
before, we subset the long distance data, but this time for trips
ending in District 1 and for non-commute trips.

We first focus on long distance trips ending to District 1. we subset
the long distance data for trips ending in District 1.
...
So `ddistrict1` now contains the non-commute long distance trips
destinationiating in District 1 with appropriate expansion weights
appended.  Now we can generate observations for modal split for
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips to District 2')
```
...
Then, we subset the non-commute long distance trips ending in
District 2.
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips to District 3')
```

Here we determine total number of non-commute long distance trips to
District 3, by mode.


First we subset the long distance non-commute trips ending in
District 3.
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips to District 4')
```


We compute total number of non-commute long distance trips to District
4, by mode.
...
## Creat subset
ddist4<-subset(wLDdata,LDDCTFIP==1|LDDCTFIP==13|LDDCTFIP==41|
                         LDDCTFIP==55|LDDCTFIP==81|LDDCTFIP==75|
                         LDDCTFIP==85|LDDCTFIP==95|LDDCTFIP==97)
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips to District 5')
```
...
## Creat subset
ddist5<-subset(wLDdata,LDDCTFIP==53|LDDCTFIP==69|LDDCTFIP==79|
                         LDDCTFIP==83|LDDCTFIP==87)
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips to Southern California')
```


We compute total number of non-commute long distance trips to Southern
California, by mode.

First we subset the long distance non-commute trips ending in Southern
California.
...
## Creat subset
oSC<-subset(wLDdata,
            LDDCTFIP==19|(LDDCTFIP==71 & LDDTRACT>=103 & LDDTRACT<5000)|
              LDDCTFIP==31|LDDCTFIP==39|LDDCTFIP==107|LDDCTFIP==29|
                LDDCTFIP==37|LDDCTFIP==59|LDDCTFIP==73|LDDCTFIP==25|
                LDDCTFIP==111|LDDCTFIP==65)
table(oSC$LDDCTFIP)
# Subset for non-commute trips
oSouthCal<-subset(oSC,!(LDTPURP==1))
table(oSouthCal$LDTPURP)
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Modal Split of Trips to Southern District 9')
```
...
## Creat subset
ddist9<-subset(wLDdata,
               LDDCTFIP==27|LDDCTFIP==51|
                 (LDDCTFIP==71 & LDDTRACT>=5000))
