...
both trips originating in and ending in Santa Cruz County. We use the
new county wide weights of the Santa Cruz County to generate weighted
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Trips to  Santa Cruz County')
```
...
that we computed in our earlier chapter on [Calculating Weights].
...
Now, we first focus on trips ending in Santa Cruz County.  Here we use
different weights depending on the origin of the trip.  Specifically,
when the origin is Santa Cruz County, we use the county-specific
weights.  When the origin is outside Santa Cruz County, we use the
statewide weights.
...
So `place_87$NTCFW` now contains the trips ending in Santa Cruz county
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Trips from  Santa Cruz County')
```


Now we perform a similar computation for trips *from* Santa Cruz
...
Here is the trip distance calculation for all trips from Santa Cruz,
by Mode.
...
And lastly for the Santa Cruz origins, here is the drive alone modal
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('##','Trip Purposes from  Santa Cruz County')
```

We start with trips from Santa Cruz County, subsetting based open the
origin county `OCTFIP=87`.
...
Santa Cruz based on new weights.
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('###','Work Trips Originating in Santa Cruz County')
```
...
```{r results='asis',echo=FALSE,cache=FALSE}
############################################################ NESTABLE SECTION
SEC('###','Work Trips Ending in  Santa Cruz County')
```


Now we want to find trips ending in Santa Cruz county `(CTFIP=87)`.
Again, we subset the place table and adjust the weights to be Santa
Cruz weights if the origins are in Santa Cruz, and to use statewide
weights if the origins are outside Santa Cruz.
