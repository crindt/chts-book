...
In this sub-section, We learn how to generate tables by mode for the following: total number of trips,
...
mode. Here, we actually generate tables for the total number of trips by
activity purpose since the trip purpose is given in the database as the
activity purpose and although many trips are associated with more than one activity, 
We consider each activity as a trip. We only consider the
...
new county wide weights of Santa Cruz County to generate weighted
