...
This document explains how person level weights are developed for City
of Los Angeles (LA) using the R statistical software for people of
City of Los Angeles who participated in the 2012 CHTS. The purpose of
these weights is to scale up information provided by respondents to
...
the adjusted weights are equal to known population totals for certain
subgroups of the population of the City of Los Angeles. Since sampling
information is not available for the City of Los Angeles, we assume
equal probability here. Variables used for raking at the person level
are:
...
more than five times the mean weight. To check whether the new weights
represent the total population of the City of Los Angeles, we do some
random checking for different variables and the results show that the
new weights align with the population of the City of Los Angeles. We
used this new person weight to calculate the final trip weight which
is an outcome of `Trip Correction Factor (TCF)` * `New Person Weight of
LA City`.
...
The following commands are used to generate person weight for City of
Los Angeles.

First we set the working directory. Then we need to import the person
data file and corresponding lookup file (containing location
information) and then we merge them.
...
We have new file which contains information of respondents from the
City of Los Angeles. We use this data file to generate person weight
for City of Los Angeles. Now, we need to recode the raking variables
to make them similar to the census data (Here we are using American
...
Here, we recode employment (`EMPLY`) and Gender variable.
...
Now we have data frame (`perdata2`) which has similar coding as census
data for raking variables. Next, we need to subset the data for
respondents who live in the City of Los Angeles. The variable named as
`HCITY` contain the city name of respondents' residential location.
...
First we import the Race variables for City of LA.
...
Then, we import the Age variables.
...
Finally, we import the Gender and Employment status variables.
...
As our data is ready for raking, next we proceed for raking to
generate person weight for the City of LA. To do this we need to
install the library "survey". First we design a survey using
`svydesign` command and we use sampling without replacement (pps
="brewer") option. Since sampling information is not available for the
City of Los Angeles, we assume equal probability here. So we don't
...
Then we perform the raking using `rake` command and check the summary
statistics of new weight.
...
After raking, we trim the weight so that very large weights were
capped to be no more than five times the mean of weight. The generated
expanded person weights for City of LA are saved in our specified
directory.
...
Now we have on file which contains new expanded person weight for the
City of LA. We want to generate new person weight (not expanded) for
the City of LA from our expanded weight. We know that `person weight` =
...
We save the new weights as a CSV file.
...
We want to merge the new weight with the City of LA person data frame
and save that.
...
Now, we have a person data file of the City of LA with new person
weight.

As we mentioned earlier we want check whether the new weights
represent the total population of the City of LA. So we perform some
random checks for different variables using following codes.
...
Here, we do a random check for the education variable.
...
The total population of the City of LA is 3,792,621 (based on
census). Our random check shows that the new weights align with the
total population of the City of LA for both variables.

Now we need to calculate the new final trip weight using the new
person weight of the City of LA. To do this, first, we need to import
the place level file and merge it with new weight file of the City of
LA.
...
The final trip weight is an outcome of `Trip Correction Factor
(TCF)` * `New Person Weight of LA City`. The follow code calculates
the final trip weight and the expanded final trip weight for the City
of LA.
...
Now we have new place data file with new trip weight for the City of
LA.
